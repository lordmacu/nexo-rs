...
async fn reactive_resolve_delivers_to_pending_sender() {
    let pending: Arc<DashMap<uuid::Uuid, oneshot::Sender<String>>> =
        Arc::new(DashMap::new());
    ...
    assert!(matches!(outcome, dispatch::__test::ResolveOutcome::Delivered));
...
async fn reactive_skips_when_no_session_id() {
    ...
    let outcome = dispatch::__test_try_resolve(&ev, &pending);
    assert!(matches!(outcome, dispatch::__test::ResolveOutcome::NotReactive));
}
...
async fn reactive_skips_when_no_pending_sender() {
    ...
    let outcome = dispatch::__test_try_resolve(&ev, &pending);
    assert!(matches!(outcome, dispatch::__test::ResolveOutcome::NoPending));
}
