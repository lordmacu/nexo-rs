...
pub fn collect_binding_errors(
    ...
    collect_binding_errors_with_providers(agents, plugins, known_tools, &KnownProviders::default())
}
...
pub fn collect_binding_errors_with_providers(
    ...
    for agent in agents {
        validate_agent_into(agent, plugins, known_tools, known_providers, &mut errors);
    }
    errors
}
...
pub fn validate_agents(
    ...
    validate_agents_with_providers(agents, plugins, known_tools, &KnownProviders::default())
}
...
pub fn validate_agents_with_providers(
    ...
    let errors =
        collect_binding_errors_with_providers(agents, plugins, known_tools, known_providers);
    if errors.is_empty() {
        return Ok(());
    }
    let mut msg = format!(
...
fn validate_agent_into(
    ...
    //
    // Phase 93.5.e — data source generalised: pulls declared
    // instances from `PluginsConfig::instances_for("telegram")`,
    // which walks the opaque `entries["telegram"]` YAML
    // sequence and extracts each entry's `instance:` field.
    // Behavior unchanged (telegram-only); the same machinery
    // can extend to any array-shape plugin once a 2nd channel
    // demands it (Phase 93.5.d trigger).
    let telegram_instances: Vec<String> = plugins.instances_for("telegram");
    for (idx, b) in agent.inbound_bindings.iter().enumerate() {
        ...
        let declared = telegram_instances.iter().any(|t| t == inst);
        if !declared {
            let known = telegram_instances.join(", ");
...
mod tests {
    ...
    fn duplicate_binding_rejected() {
        ...
        let err = validate_agent(
            &a,
            &plugins_with_telegram(tg.clone()),
            &KnownTools::default(),
        )
        .unwrap_err();
        assert!(matches!(
    ...
    fn unknown_tool_rejected_when_catalogue_supplied() {
        ...
        let err =
            validate_agent(&a, &plugins_with_telegram(Vec::<String>::new()), &tools).unwrap_err();
    ...
    fn wildcard_tool_always_passes() {
        ...
        validate_agent(&a, &plugins_with_telegram(Vec::<String>::new()), &tools)
    ...
    fn trailing_star_glob_matches_prefix() {
        ...
        validate_agent(&a, &plugins_with_telegram(Vec::<String>::new()), &tools)
            .expect("prefix glob should match");
    }
    ...
    fn collect_binding_errors_aggregates_multi_agent_problems() {
        ...
        let errors = collect_binding_errors(
            &[a1, a2],
            &plugins_with_telegram(Vec::<String>::new()),
            &KnownTools::default(),
        );
    ...
    fn validate_agents_aggregates_into_single_error_message() {
        ...
        let tg: Vec<String> = vec!["ana_tg".into()];
        let err =
            validate_agents(&[a], &plugins_with_telegram(tg), &KnownTools::default()).unwrap_err();
        let msg = format!("{err:#}");
    ...
    fn unknown_provider_rejected_when_catalogue_supplied() {
        ...
        let err = validate_agents_with_providers(
            &[a],
            &plugins_with_telegram(Vec::<String>::new()),
            &KnownTools::default(),
            &known,
    ...
    fn happy_path_with_multiple_checks() {
        ...
        let tg: Vec<String> = vec!["ana_tg".into()];
        let tools = KnownTools::new(["whatsapp_send_message", "weather"]);
        validate_agent(&a, &plugins_with_telegram(tg.clone()), &tools)
