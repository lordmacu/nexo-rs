...
async fn handle_child_publish(bridge: &BridgeContext, plugin_id: &str, parsed: &Value) {
    ...
    // Reserved request-reply inbox topics (`_inbox.<uuid>`) are
    // generated by the daemon's `broker.request` call site for
    // each in-flight RPC. The plugin can't declare these in its
    // manifest allowlist because the UUID is daemon-side and
    // ephemeral; bypass the gate so JSON-RPC replies route back
    // to the requesting handler.
    let is_reply_inbox = topic.starts_with("_inbox.");
    if !is_reply_inbox
        && !bridge
            .publish_allowlist
            .iter()
