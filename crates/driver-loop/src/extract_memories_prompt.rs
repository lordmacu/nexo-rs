...
/// be the output of `scan_memory_manifest()`.
pub fn build_extract_prompt(new_message_count: u32, existing_memories: &str) -> String {
    let manifest_block = if existing_memories.is_empty() {
        String::new()
    } else {
        format!(
    ...
    [
        opener(new_message_count, &manifest_block),
        String::new(),
...
fn opener(new_message_count: u32, manifest: &str) -> String {
    [
        format!(
...
fn memory_frontmatter_example() -> &'static str {
    "\
```markdown
---
name: {{memory name}}
...
fn types_section() -> String {
    [
        ...
        "    <examples>",
        r#"    user: check the Linear project "INGEST" if you want context on these tickets, that's where we track all pipeline bugs"#,
        r#"    assistant: [saves reference memory: pipeline bugs are tracked in Linear project "INGEST"]"#,
        "",
...
fn what_not_to_save_section() -> String {
    [
