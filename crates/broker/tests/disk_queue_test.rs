...
async fn disk_queue_applies_backpressure_over_halfway() {
    ...
    for i in 0..5 {
        queue.enqueue("test.topic", &dummy_event(&format!("{i}"))).await.unwrap();
    }

    // At the halfway mark — next enqueue should still be near-instant.
    let t0 = Instant::now();
    queue.enqueue("test.topic", &dummy_event("halfway+1")).await.unwrap();
    let fast = t0.elapsed();
    assert!(
    ...
    for i in 0..3 {
        queue.enqueue("test.topic", &dummy_event(&format!("fill-{i}"))).await.unwrap();
    }
    let t1 = Instant::now();
    queue.enqueue("test.topic", &dummy_event("near-cap")).await.unwrap();
    let slow = t1.elapsed();
    assert!(
