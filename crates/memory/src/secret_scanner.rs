...
/// Convert a rule ID (kebab-case) to a human-readable label with
/// correct capitalization for known entities.
fn rule_id_to_label(rule_id: &str) -> String {
    // Words whose canonical capitalization differs from title case.
    let special: &[(&str, &str)] = &[
        ("ad", "AD"),
        ("api", "API"),
        ("aws", "AWS"),
    ...
    rule_id
        .split('-')
        .map(|part| {
            match special.iter().find_map(|(key, val)| if *key == part { Some(*val) } else { None }) {
                Some(special_val) => special_val.to_string(),
                None => {
...
/// Return the `&'static str` label for a known rule ID. Panics if
/// the rule ID is unknown (should never happen — all rule IDs are
/// compile-time constants).
fn static_label(rule_id: &str) -> &'static str {
    match rule_id {
