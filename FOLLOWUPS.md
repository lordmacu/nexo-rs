...
### Phase 96 — Poller Laravel refactor + builtins extraction (in-flight)   ⬜ ACTIVE

Phase 96 ships V2 Laravel-style poller contract + `[plugin.poller]`
manifest section + daemon-side reverse-RPC handler + extracts
provider-specific builtins to standalone subprocess plugin repos.

Open items recorded during execution (Block A + B done as of
this commit, Blocks C-F still pending):

- **`nexo-poller-ext` deletion (cycle 3).** Crate is
  `#[deprecated(since = "0.2.0")]` and README carries a WARN
  header. Plan: after 2 release cycles (Phase 97 + 98) with zero
  download telemetry, delete the crate entirely. Today: dep is
  intact, still functional for any operator with extension-based
  pollers in flight.

- **PluginPollerHost `metric_inc` Prometheus aggregator.** Daemon
  reverse-RPC handler today forwards subprocess `metric_inc` calls
  to `tracing::info!` only. Wire into the daemon's Prometheus
  registry so subprocess pollers can fan out arbitrary counters
  into `/metrics`. Blocked on a generic named-counter API in
  `nexo-poller::telemetry` (currently only per-kind hardcoded
  metrics).

- **Plugin poller supervisor (Phase 96.8.a).** Today plugin v2
  poller subprocesses inherit the existing `SubprocessNexoPlugin`
  supervisor (Phase 81.21.b.b lineage); the reverse-RPC subscriber
  is a tokio task that survives child restart, and broker request
  timeouts surface as `Transient` errors the runner retries. A
  dedicated supervisor with backoff per kind + manual restart RPC
  is a follow-up if the inherited path proves insufficient under
  real-world subprocess crash patterns.

- **Block D — extract `nexo-rs-poller-{gmail,google_calendar,rss}`
  to standalone public repos + crates.io publish.** Three repos
  scaffold + port logic + publish per Phase 94/95 pattern.
  Includes OAuth fcntl-lock concurrency mitigation for the two
  Google pollers (gmail + google_calendar both touch the same
  agent token file).

- **Block E — ephemeral lifecycle (96.E1-E4).** `[plugin.poller]
  lifecycle = "ephemeral"` is parsed and validated by the manifest
  layer but the daemon currently only handles `long_lived`. Spawn-
  per-tick path requires new stdio JSON-RPC primitives (no broker
  subscription, direct stdin/stdout dispatch + SIGTERM on reply).
  Decision locked during /forge plan as Block E (4 sub-phases);
  not yet implemented.

- **Block F — docs (`recipes/poller-plugin.md` +
  `architecture/poller-v2.md` + update `build-a-poller.md`).**
  Operator how-to for the manifest section + design rationale of
  the Laravel-style cut + V2 builder recipe. Blocked on Block D
  completion so the docs can reference live plugin repos as
  examples.

### Phase 95 — Web-search plugin extraction (close-out follow-ups)   ⬜ ACTIVE

Phase 95 shipped 2026-05-17 with all sub-phases ✅ and three
crate releases (microapp-sdk 0.1.19, nexo-core 0.2.0,
nexo-plugin-web-search 0.1.0). Standalone repo at
https://github.com/lordmacu/nexo-rs-plugin-web-search (master,
6 commits, tag `nexo-plugin-web-search-v0.1.0`, 54/54 nextest).
Open items:

- **`expand` link-extraction post-processor.** v0.1.0 accepts
  `args.expand = true` but returns `expand_supported: false`
  hint (subprocess can't reach daemon's `link_extractor`).
  Follow-up adds a daemon-side post-processor: when
  `policy.expand_default = true` AND the plugin reply contains
  URL list, daemon runs `link_extractor.fetch` itself for the
  top-3 URLs and attaches bodies. Generic — same hook serves
  any future subprocess tool returning URLs.

- **`admin/cache_clear` real implementation.** ✅ SHIPPED
  2026-05-17 in `nexo-web-search 0.2.0` (added
  `WebSearchCache::clear() -> Result<u64>` +
  `WebSearchCache::stats() -> Result<CacheStats>`) and
  `nexo-plugin-web-search 0.1.1` (wired into
  `admin/cache_clear` + `admin/cache_stats` RPCs).

- **`provider_status` breaker state surfacing.** ✅ SHIPPED
  2026-05-17 in `nexo-web-search 0.2.0` (added
  `WebSearchRouter::provider_states() -> Vec<ProviderState>`
  where `available: bool` reflects per-provider breaker state)
  and `nexo-plugin-web-search 0.1.1` (wired into
  `admin/provider_status` RPC).

- **Streaming progress (`WebSearchProgress`).** Today's
  `tool.invoke` JSON-RPC is request/reply only. Add streaming
  SDK contract (`tool.invoke.stream` method + per-chunk
  partials) for tools that benefit from progressive output.
  Useful for web_search (per-result snippets stream as they
  arrive from upstream) + future research / agent-handoff
  tools.

- **`web_research` deep-search bundle (separate plugin).** A
  composite tool that runs N parallel `web_search` queries
  with different angles + summarises via LLM. Should ship as
  its own subprocess plugin (`nexo-plugin-web-research`) that
  depends on `nexo-plugin-web-search` for the underlying search
  calls. Demonstrates plugin-to-plugin composition via broker.

- **Per-provider plugin variant (OpenClaw pattern).** Some
  operators want only one provider (e.g. only Brave for
  compliance reasons). Today they rebuild
  `nexo-plugin-web-search` with `--features brave` only. Future
  per-provider standalone binaries (`nexo-plugin-brave-search`,
  etc.) would let operators install just what they need without
  building from source. Lower priority — feature flags work for
  now.

- **`nexo-poller` migration to published web-search.** Mirror
  the pending google poller follow-up: if `nexo-poller` ever
  adds a builtin that calls `WebSearchRouter` directly, switch
  to crates.io `nexo-web-search = "0.1.2"` (now possible since
  the dep was removed from `crates/core` but the lib stays as
  workspace member). Currently no poller builtin uses it; flag
  only if one is added.

### Phase 94 — Google plugin extraction (close-out follow-ups)   ⬜ ACTIVE

Phase 94 shipped 2026-05-16 with 11/11 sub-phases ✅. Standalone
repo at https://github.com/lordmacu/nexo-rs-plugin-google
(master, 5 commits, tag `nexo-plugin-google-v0.2.0`, 59/59 nextest).
Published as `nexo-plugin-google 0.2.0` on crates.io. Open items:

- **94.9 — Publish to crates.io.** ✅ SHIPPED 2026-05-16.
  `cargo install nexo-plugin-google` works.

- **Poller migration to published `nexo-plugin-google` lib.**
  ✅ SHIPPED 2026-05-16. `nexo-poller` switched to
  `nexo-plugin-google = "0.2.1"` (crates.io). In-tree
  `crates/plugins/google/` directory deleted entirely.
  Workspace pulls google lib only from crates.io now.

- **PKCE migration.** ✅ SHIPPED 2026-05-17 in
  `nexo-plugin-google 0.3.0`. `generate_pkce_pair()` produces
  RFC 7636 verifier + S256 challenge; loopback flow stashes
  verifier in `pending_verifier` (RwLock); `exchange_code`
  submits `code_verifier`. `build_auth_url(state)` preserves
  back-compat (no PKCE); `build_auth_url_with_pkce(state, Some(c))`
  is the new variant.

- **Multi-account-per-agent.** ✅ SHIPPED in `nexo-plugin-google
  0.2.1`. `DashMap<account_id, _>` + `by_agent: DashMap<agent_id,
  Vec<account_id>>` lookup. Tools accept optional `account:`
  arg. Multi-account-per-tenant (SaaS shape) still deferred —
  same accounts-array works per agent, no tenant key yet.

- **`[plugin.http]` route mount.** ✅ SHIPPED 2026-05-17 in
  `nexo-plugin-google 0.3.0`. `[plugin.http] mount_prefix =
  "/google"` proxies via `plugin.google.http.request` broker
  handler. Routes: `GET /google/status` (plugin metadata +
  per-account oauth listing) + `GET /google/health`. 404 for
  unknown paths.

- **`[plugin.metrics]` Prometheus scrape.** No useful counters
  today; add refresh_token rotation count, `google_call` rate,
  circuit-breaker open events when production traffic justifies.

### Phase 81.20.x Stage 7 Phase 2 — whatsapp orchestration cleanup   ✅ SHIPPED 2026-05-16

Bucket C of the Phase 93 decoupling audit. After Stage 7 Phase 1
(commit `136db75a`) the daemon's tool + pairing-adapter
fallbacks for whatsapp + telegram are gone.

**Shipped this wave (2026-05-16):**

- ✅ Hardcoded `/whatsapp/*` HTTP block, `dispatch_route`,
  `WhatsappRoute` variants, `PAIR_PAGE_HTML` — all deleted.
  `PluginHttpRouter` (Phase 81.33.b.real Stage 2) forwards
  `/whatsapp/*` to the subprocess.
- ✅ `RuntimeHealth.wa_pairing` field + `SharedPairingState`
  boot-time construction + per-instance init — deleted.
  Subprocess owns its own `WhatsappPlugin::pairing` field.
- ✅ `spawn_whatsapp_pairing_state_subscriber` — deleted.
- ✅ Auto-tunnel block (`wa_tunnel_cfg` + `/whatsapp/pair`
  auto-spawn) — deleted. Replaced by the generic
  `[plugin.public_tunnel]` manifest section + capability-gated
  daemon iterator (`NEXO_PLUGIN_PUBLIC_TUNNEL_ALLOW=1`).
- ✅ `QrSnapshot` mirroring logic — deleted.
- ✅ `[plugin.pairing.trigger]` manifest section
  (`start_method` + `cancel_method` + optional `timeout_seconds`).
  Validator gates blank methods + non-QR kind.
- ✅ `BrokerPairingTrigger` in `nexo-core/src/agent/admin_rpc/`
  + `spawn_pairing_inbound_subscriber` helper (13 tests).
- ✅ `PairingChannelTriggers` refactored to `Arc<DashMap>`
  newtype so the daemon can insert entries post-bootstrap
  (after `wire_plugin_registry` returns with manifests loaded).
- ✅ Daemon boot loop in `src/main.rs` registers
  `BrokerPairingTrigger` from each plugin's manifest, spawns
  one pairing-inbound subscriber per channel; legacy hardcoded
  `WhatsappPairingTrigger::from_configs` registration coexists
  (overwrite by channel_id when plugin v0.4.4 ships the
  manifest section).
- ✅ `[plugin.public_tunnel]` manifest section + validator + 5
  tests. Capability-inventory entry for
  `NEXO_PLUGIN_PUBLIC_TUNNEL_ALLOW`.
- ✅ Docs: `manifest-pairing-trigger.md` + `manifest-public-tunnel.md`
  + SUMMARY.md + mdbook clean.

Workspace nextest: 6415/6415 + plugin-manifest 186/186.

**Plugin-side migration (out-of-tree, `nexo-rs-plugin-whatsapp`):**

- v0.4.4 manifest declares `[plugin.pairing.trigger]`.
- `src/pairing_admin.rs` serves `nexo/admin/whatsapp/pairing/start`
  + `/cancel` admin RPCs; publishes QR + state to
  `plugin.inbound.whatsapp.<inst>.pairing.{qr,state}`.
- Published chain: `nexo-plugin-manifest 0.1.6`
  + `nexo-plugin-whatsapp 0.4.4` (both on crates.io).

**Daemon-side close-out (this commit):**

- `WhatsappPairingTrigger::from_configs` registration block
  removed from `src/main.rs`. Manifest-driven
  `BrokerPairingTrigger` is now the sole `pairing/start` path
  for the whatsapp channel.
- `plugin-whatsapp` dropped from default features.
- `cargo tree -i nexo-plugin-whatsapp -p nexo-rs` returns
  "did not match any packages" in the default build. Operators
  needing the legacy embedded `WhatsappPlugin` Rust handle can
  still opt in via `--features plugin-whatsapp`.

**Residual follow-ups — all resolved 2026-05-16:**

- ✅ `plugin-whatsapp` + `plugin-telegram` + `plugin-browser`
  features deleted. Test fixtures rewritten to use
  `serde_yaml::Value` literals. `cargo tree -i nexo-plugin-*`
  unmatched for all four canonical plugins (whatsapp, telegram,
  email, browser).
- ✅ `nexo-whatsapp-pair` standalone crate proposal closed as
  obsolete. Setup wizard's `run_whatsapp_pairing_sync` already
  spawns `nexo-plugin-whatsapp --pair-once <session_dir>` via
  PATH (Phase 81.20.x Bucket C2 BC.6); zero Cargo coupling on
  the plugin crate. A separate pairing-only binary would
  duplicate code without improving the operator workflow
  (`cargo install nexo-plugin-whatsapp` is still the runtime
  install path either way).

### Phase 81.20.x — autonomous worker channel send (broker-RPC bridge)   ✅

**Shipped 2026-05-16.** The original
`RemoteToolHandler`-via-registry approach was reconsidered: the
mcp-server runs out-of-process from the daemon (separate
binary subcommand), so the daemon-local `RemoteToolHandler`
(STDIN/STDOUT pipes to the subprocess) is unreachable. The
fix used the **broker contract** instead:

- `nexo-core` gained `PluginChannelSendTool`
  (`crates/core/src/agent/plugin_send_tool.rs`), an agnostic
  LLM tool that wraps an `Arc<dyn ChannelOutboundDispatcher>`.
  Args: `{ channel, account_id, to, body, msg_kind?,
  reply_to_msg_id?, attachments? }`. Routes by `channel` —
  no plugin-specific knowledge baked in.
- `crates/setup/src/admin_bootstrap.rs` now registers the
  `EmailTranslator` + `TelegramTranslator` (+ cfg-gated
  `WhatsAppTranslator`) alongside the broker dispatcher,
  so admin RPC `processing.intervention.reply` and the
  worker tool both cover all canonical channels.
- `start_mcp_autonomous_worker`
  (`proyecto/src/main.rs:14256`+) builds a
  `BrokerOutboundDispatcher` with the same translator set
  and registers `PluginChannelSendTool` in `worker_tools`.
  The worker publishes to the same broker the plugin
  subprocess subscribes to — no daemon routing required.

Test coverage: 7 unit tests on `PluginChannelSendTool`
(build_message defaults, attachments, error paths, dispatcher
contract, schema shape). Workspace nextest: 6382/6382 pass.

### Phase 93 Bucket D — override (email moved to subprocess)

The Phase 93 decoupling audit memo
(`docs/src/architecture/phase-93-decoupling-audit.md`, 2026-05-15)
recommended Bucket D's 24 sites as **intentional design choice**
("email stays in-process forever"). User request on 2026-05-16
overrode the recommendation in favour of full subprocess
decoupling, citing "framework should have zero plugin-specific
knowledge". Phase 81.20.x F0-F5 (shipped 2026-05-16) closed
Bucket D entirely. The audit memo's recommendation stands as
historical reference but no longer reflects current architecture.

### Plugin auto-detection cache (cargo-install ergonomics)   ⬜ DEFERRED

Boot-time discovery now spawns each `nexo-plugin-*` binary in
`search_paths` with `--print-manifest` (2s timeout) to extract
the embedded manifest. With N=5 plugins and ~20ms per probe the
total cost is ~100ms — under the LLM-bound startup noise floor,
so caching deferred.

Scope when implemented:

- Cache key: `(canonical_path, mtime, size)` per probed binary.
- Persistence: `<state_root>/plugin-discovery-cache.json`.
- Read at boot before spawning; on cache hit, skip probe and
  reuse cached manifest TOML.
- Invalidate by mtime/size change. Force-refresh CLI flag
  (`agent --refresh-plugin-cache`).
- Diagnostic when cache entry stale vs binary missing.

Trigger to implement: any benchmark showing daemon boot >300ms
attributable to plugin probing, OR N exceeds ~15 plugins.

### Email plugin — migrate to multi-instance × multi-account   ✅ RESOLVED 2026-05-16 (Wave 1)

Wave 1 shipped 2026-05-16 in `nexo-plugin-email 0.5.0`. Plugin-side
multi-tenant contract complete: array config shape, instance_registry,
boot::apply_configure with diff-aware reload, admin RPC list_tenants,
manifest workspace_walk dashboard, 275/275 nextest green.

Daemon-side subprocess flip (Wave 2) tracked as a separate
follow-up below.

Original scope (8 items) — all delivered in Wave 1 except #3 and
#4 which are daemon-side concerns:

1. ✅ Manifest — `shape = "array"` + `dashboard.layout =
   workspace_walk subdir = "email"` + `auth_check =
   session_dir_files` with multiple candidate paths.
2. ✅ Config — `EmailPluginConfig` gains `instance: Option<String>`
   + `allow_agents: Vec<String>`; `EmailPluginConfigFile.email` is
   the `Single | Many` shape enum.
3. ⏸️ Subprocess seeding — daemon-side concern, deferred to Wave 2
   (`seed_email_subprocess_env_for` promoted from `#[cfg(test)]`
   to production + per-tenant env stamping).
4. ⏸️ `SubprocessNexoPlugin` — daemon-side concern, deferred to
   Wave 2 (swap `email_arc.clone()` in-process factory for
   `register_instance_subprocess_factories("email", ...)`).
5. ✅ Dashboard — manifest declares `workspace_walk subdir =
   "email"`.
6. ✅ Auth check — `session_dir_files` candidates
   `["email_password.txt", ".nexo-paired"]`.
7. ✅ Tools — Step 5 no-op on plugin side (email subprocess
   advertises zero tools; daemon's in-process ToolRegistry handles
   dispatch). Multi-tenant agent routing arrives with Wave 2 via
   `instance_registry::lookup(label)`.
8. ✅ Bump — `nexo-plugin-email` 0.4.0 → 0.5.0.

Today `nexo-rs-plugin-email` is **single-instance multi-account
internal**: one subprocess hosts N IMAP/SMTP accounts via
`EmailPluginConfig.accounts: Vec<EmailAccountConfig>`. Works
fine when accounts belong to one operator/tenant — shared infra
is the right trade-off.

Doesn't fit the SaaS multi-tenant model (microapp creator) where
distinct tenants (empresa A, empresa B) must NOT share process
state, log streams, or blast radius. One tenant's IMAP outage
shouldn't stall another tenant's IDLE loop.

Goal: **multi-instance × multi-account** — one subprocess per
tenant, each subprocess still owns N accounts internally. Best
of both worlds: cross-tenant isolation at the process boundary
plus intra-tenant resource sharing for related accounts.

Shape:

```
N subprocesses (one per tenant)
├── instance "empresa-A" — accounts: [support@A, ventas@A, marketing@A]
├── instance "empresa-B" — accounts: [hola@B, info@B]
└── instance "empresa-C" — accounts: [contacto@C]
```

Scope:

1. **Manifest** — `[plugin.config_schema] shape = "object" →
   "array"`. Each entry has `instance: String` (tenant id) +
   keeps the existing `accounts: Vec<EmailAccountConfig>` inside.
2. **Config** — `EmailPluginConfig` gains `instance: String`;
   external wrapper becomes `Vec<EmailPluginConfig>`.
3. **Subprocess seeding** — `seed_email_subprocess_env_for` in
   daemon stamps per-instance `NEXO_PLUGIN_EMAIL_INSTANCE` +
   per-instance config/secrets paths so each subprocess loads
   only its slice of accounts.
4. **`SubprocessNexoPlugin`** — spawn one binary per
   instance, supervisor tracks each independently (already the
   pattern for telegram/whatsapp).
5. **Dashboard** — `[plugin.dashboard.layout] kind = single →
   workspace_walk subdir = "email"` so the wizard surfaces
   per-tenant entries.
6. **Auth check** — keep `file_presence` but per-instance secrets
   path (`secrets/<instance>/email_password.txt` instead of
   `secrets/email_password.txt`).
7. **Tools** — `email_*` tools gain implicit instance binding
   via agent-to-instance mapping (same pattern whatsapp uses
   today).
8. **Bump** — `nexo-plugin-email` 0.4.x → 0.5.0 (breaking
   manifest + config shape).

Migration shim: legacy single-instance config (bare
`EmailPluginConfig` not wrapped in Vec) auto-promoted to
`vec![EmailPluginConfig { instance: "default", ...rest }]`.

### Browser plugin — migrate to multi-instance   ✅ RESOLVED 2026-05-15

Shipped in `nexo-plugin-browser 0.3.0` (branch
`feat/multi-instance-step1` in
`/home/familia/chat/nexo-rs-plugin-browser/`). Twelve atomic
commits, each tested in isolation before next; 130/130 nextest
green at the end (2 ignored, both gated on `CHROMIUM_BIN`).

Delivered (all 7 scope items):

1. ✅ **Manifest** — `shape = "array"` + 6 new Stages 1-6 sections
   (`pairing`, `pairing.adapter`, `pairing.instructions`,
   `pairing.fields`, `http`, `admin`, `metrics`,
   `dashboard.layout workspace_walk subdir = browser`,
   `dashboard.auth_check session_dir_files` led by `.nexo-paired`
   sentinel) + `capabilities.broker.subscribe` allowlist +
   `supervisor.respawn = true`.
2. ✅ **Config** — `BrowserPluginShape::{Single, Many}` untagged
   enum, `BrowserConfig.instance: Option<String>` +
   `allow_agents: Vec<String>`, back-compat shim normalises
   single-map shape to a 1-element vec.
3. ✅ **Runtime** — `instance_registry` (`OnceLock<Arc<DashMap<
   String, Arc<BrowserPlugin>>>>`) mirroring telegram/whatsapp
   patterns. `boot::apply_configure` populates + diffs +
   unregisters stale labels with `shutdown_chrome().await`.
4. ✅ **Tool dispatch** — every `browser_*` tool's input schema
   gains optional `instance: string`. `resolve_plugin_or_legacy`
   implements the 4-case routing matrix (explicit / implicit-1 /
   implicit-0 legacy fallback / implicit-N ambiguous) + a
   per-instance `allow_agents` gate.
5. ✅ **Sandbox** — `fs_write_paths = ["${state_dir}"]` (parent);
   boot loop resolves `${state_dir}/instances/<label>/` per
   instance. Daemon-side per-spawn path substitution (Phase 81.22)
   confirmed by manual probe; if missing in some environments,
   `browser.81.22.runtime-path-subst` opens.
6. ✅ **Tests** — 130 total: `config_migration` (7),
   `dispatch_routing` (8), `configure_boot` (8), `manifest_parse`
   (8), `e2e_multi_instance` JSON-RPC wire (3),
   `e2e_multi_instance_isolation` Chrome cookie isolation (1,
   `CHROMIUM_BIN`-gated), plus inline unit tests on
   `instance_registry` (7), `auto_discovery` handlers (13),
   `metrics` (5).
7. ✅ **Bump** — Cargo.toml `0.3.0` shipped (pre-set by Phase
   93.4.e). CHANGELOG.md rewritten consolidating 93.4.e + this
   migration under one 0.3.0 entry.

Additional infrastructure delivered alongside (Phase 81.33.b.real
Stages 1+2+4+5+6 handler half):

- `src/admin.rs` — 5 admin RPC verb handlers (`list_instances`,
  `shutdown`, `restart`, `mark_paired`, `launch_visible`).
- `src/auto_discovery.rs` — pairing / http / admin / metrics
  broker-dispatch handlers (pure functions, testable in-process).
- `src/metrics.rs` — per-instance Prometheus counters /
  histograms / gauges; wired into `main.rs::on_tool` so every
  dispatch records latency + outcome under
  `instance="<label>"`.

Deferred (out-of-scope, opened as follow-ups):

- `browser.auto_discovery.subscriber` — the broker subscriber loop
  that actually pumps requests through the
  `auto_discovery::*` handlers. Plugin doesn't construct a broker
  handle today; manifest declarations let daemon-side helpers
  route correctly, but plugin acks are no-op until the subscriber
  wiring lands. Mirrors the spawn-broker ceremony in
  `nexo-rs-plugin-telegram/src/main.rs:231-371`.
- `browser.launch_visible.runtime` — make admin `launch_visible`
  actually flip `headless = true → false` on the next Chrome
  boot. Today the verb is a no-op acknowledgement so the wire
  contract is in place.
- `browser.per-instance-supervisor` — one subprocess still hosts
  all instances; a single Chrome crash + plugin-process panic
  resets all of them. Per-instance supervisor (one Chrome crash
  isolated from siblings) is future scope.
- `browser.0.4.0.deprecate-legacy-per-agent` — flip
  `NEXO_PLUGIN_BROWSER_LEGACY_PER_AGENT` default from `1` to `0`
  once enough operators have migrated.
- `browser.81.22.runtime-path-subst` — open only if a sandbox
  probe surfaces that bubblewrap path substitution is
  manifest-time-only (not per-spawn). Boot loop currently writes
  resolved paths into each `BrowserConfig.user_data_dir`, so
  Chrome spawns with the literal path either way; this follow-up
  matters only if the daemon-side sandbox enforcement layer
  rejects late-bound paths.

Cost: ~200-500MB RAM per Chrome instance. Operators with N agents
using the browser plugin will see RAM scale linearly. Documented
in the plugin README so single-instance mode (one profile shared)
stays an explicit choice via the legacy single-map YAML shape.

### Phase 81.33.b.real Stage 7 — drop legacy hardcoded fallback paths   ⬜ DEFERRED-strict

After Phase 81.33.b.real v0.4 (telegram + whatsapp + email all
published with full 5-section manifests + real broker handlers,
2026-05-15), the daemon still keeps **two parallel code paths**
per canonical plugin: the lib-linked direct calls (feature-on
in-process mode) and the manifest-driven broker-dispatch (
subprocess + auto-discovery). Both work; the dispatch one is the
forward path for new plugins.

Legacy sites still active in `proyecto/src/main.rs`:

- `WhatsappPairingAdapter::new(broker.clone())` registration (L1720) +
  `TelegramPairingAdapter::new(broker.clone())` (L1724) — bypassed by
  `GenericBrokerPairingAdapter` if the plugin manifest declares
  `[plugin.pairing.adapter]`.
- Hardcoded `/whatsapp/*` HTTP block (L15539) — bypassed by
  `PluginHttpRouter` if manifest declares `[plugin.http]`.
- `with_wa_bot_handle` admin RPC — superseded by `PluginAdminRouter`
  when manifest declares `[plugin.admin]`.
- `nexo_plugin_email::metrics::render_prometheus(...)` direct lib
  call (L15413) — superseded by `plugin_metrics::scrape_all` when
  manifest declares `[plugin.metrics]`.
- `TelegramDashboardSource` + `WhatsappDashboardSource` +
  `EmailDashboardSource` in `setup/src/services/channels_dashboard.rs`
  (L88/L104/L159) — superseded by `dashboard_sources_from_manifests`.

Trigger to retire:

1. proyecto bumps the canonical plugin pins to ≥0.4.0 once the
   3 plugins have re-published with matching `nexo-config`,
   `nexo-core`, `nexo-pairing` versions (current 0.4.0 publishes
   pin 0.1.x, so the lib-linked workspace can't consume them
   without a duplicate-crate type-mismatch — see the rollback
   note below).
2. Verify subprocess + manifest dispatch path is in steady-state
   prod for at least one release cycle.
3. Remove the legacy sites in one PR + bump nexo-core minor.

Blocked-by during 2026-05-15 session: the version-skew (plugins
publish with `nexo-config = "0.1.1"` pin while proyecto runs
`nexo-config 0.1.8`) prevents bumping proyecto pins to 0.4.x
without breaking lib-linked compile. Resolving requires either
re-publishing the 3 plugins with updated workspace-dep pins or
exposing them via path-override.



### Phase 92 — `nexo-tunnel-quick` — shipped 2026-05-14, publish queued

Phase 92 closed 12/12 in PHASES.md (92.0 spike + 92.1 scaffold +
92.7 supervisor surface + 92.8 `nexo-tunnel` re-export shim +
92.10 lifecycle metrics + docs + 92.2-92.6/92.9/92.11
closed-as-covered per scope shift to *workspace wraps Cristian's
upstream `cloudflare-quick-tunnel`*).

Manual steps deferred to a publish session:

1. `cargo release patch -p nexo-tunnel-quick --execute` (first
   crates.io upload, 0.1.0).
2. `cargo release patch -p nexo-tunnel --execute` (shim, 0.3.0)
   — depends on step 1 being live so the path-dep → crates.io
   resolution succeeds.
3. Announce `nexo-tunnel` retirement timeline once a release
   cycle has shipped with the shim active. Phase 92.11 marked
   "ready" in PHASES.md; the actual deprecation timer lives
   here so it survives the close-out.

Open follow-up after publish:

- ~~Wire `render_prometheus_for(&[handles]).await` into the
  daemon's `/metrics` aggregator next to the memory / LLM /
  web-search exposers.~~ Lifecycle counters shipped 2026-05-15
  (PR #57) — `tunnel_starts_total`,
  `tunnel_starts_failed_total{reason}`, `tunnel_shutdowns_total`
  now surface from `handle_metrics_conn`. Per-tunnel supervisor
  counters (`tunnel_streams_total`, `tunnel_bytes_in/out_total`,
  `tunnel_reconnects_total`) need a `TunnelHandle` registry on
  `RuntimeHealth` — the two creation sites
  (`src/main.rs:3815` whatsapp-pairing + `src/main.rs:10550`
  admin `--tunnel`) move the handle into local-scope teardown
  flows. ~~Routing them through a shared registry is the
  deferred 92.followup.b.~~ **Shipped 2026-05-15 (PR #58).**
  `RuntimeHealth.tunnel_registry: Arc<RwLock<Vec<Arc<TunnelHandle>>>>`
  populated by the whatsapp-pairing creation site; aggregator
  snapshots refs + calls `render_prometheus_for` so per-tunnel
  counters now surface labelled by `tunnel_id`. Admin
  `--tunnel` CLI standalone site intentionally out-of-registry
  (no daemon `/metrics` in that subcommand).

### Phase 81.33.e ✅ partial (2026-05-14)

Shipped: generic `register_instance_subprocess_factories<C>` helper
consolidating telegram + whatsapp subprocess-flip blocks. -124
LOC duplicated body + +137 LOC helper (net +13 but zero duplication).
Plugin ids still appear as string literals in call sites
(`"telegram"` / `"whatsapp"`) — full discovery-iteration loop
blocked on Phase 81.33.f (opaque per-plugin config).

### Phase 81.36 — plugin-driven setup wizards (FUTURE, multi-week)

Setup CLI (`crates/setup/`) currently hardcodes per-channel
wizard flows (`run_whatsapp` / `run_telegram` / `run_email` /
`run_google`) + the dispatch array
`["telegram", "whatsapp", "email"]` at
`crates/setup/src/services/channels_dashboard.rs:235`.

Each per-channel function is bespoke UX (QR scan, token +
chat_id capture, IMAP probes, OAuth device flow). Making this
generic requires:

- Plugin SDK trait `SetupWizard` with methods like
  `present_choice(&Prompt) -> Choice`,
  `collect_credentials(&Schema) -> Json`,
  `verify_connection(&Json) -> Result<()>`,
  `persist(&Json) -> Result<()>`.
- Plugin-side impls exposed via subprocess RPC
  (`setup.configure`) so the daemon-side CLI calls them
  generically.
- Plugin manifest declares
  `[plugin.setup_hint] suggest_link = true; label = "..."`.
- Setup CLI iterates `plugin_handles` filtered by setup_hint
  + dispatches to RPC. Zero hardcoded `if id == "X"` branches.

Risks:
- Per-channel UX quality drops if plugins ship bad setup flows
  (no curation gate).
- Subprocess RPC roundtrip in interactive prompts adds latency.
- Existing curated UX (e.g. WhatsApp QR sub-process pairing)
  must be reimplemented behind the SDK trait.

Scope: 2-3 weeks dedicated. Brainstorm + spec + plan pending.
Sequencing: AFTER Phase 81.33.b.real (manifest-driven pairing
adapter) lands, since both touch plugin lifecycle declarations.

### Phase 81.33.b ✅ partial (2026-05-14)

Shipped: trait method + helper consolidation. 6 hardcoded
sites → 1 (`build_known_pairing_registry`). Next step
(81.33.b.real) is manifest-driven `GenericBrokerPairingAdapter`
on `SubprocessNexoPlugin::build_pairing_adapter`, but blocked
on auth-model design (same blocker as 81.33.a step 5).

### Phase 81.33.b.real — manifest-driven pairing adapter   ⬜ DEFERRED-strict

Re-evaluated 2026-05-15. The "NEXT SESSION" framing was
written when the assumption was a 5h-clean manifest schema
existed. Investigation in this session showed the
`normalize_sender` per-channel custom logic does NOT
generalise cleanly via manifest data — the `topic_template`
+ `normalize_regex_or_passthrough` + `markdown_v2_escape`
draft from the original resume prompt covers telegram's
shape but breaks for whatsapp (E.164 + `@c.us` strip is not
a single regex) and would shatter for any 3rd-party channel
with E.164-with-region rules (sms carriers) or DH-key
handshakes (signal-style).

**See full DEFERRED-strict rationale + trigger** in the
"Phase 81.33.b.real — manifest-driven pairing-adapter
schema" section below (next to Phase 93 entries).

**Status today.** The two hardcoded constructor blocks in
`build_known_pairing_registry` are already
`#[cfg(feature = "plugin-X")]`-gated by Phase 93.12.a + c.2.
Slim daemon binary drops both cleanly. 3rd canonical
channel = one more cfg-block (linear scaling).

Resume prompt for the broader 81.33 work:

> Phase 81.33.b — add `NexoPlugin::build_pairing_adapter(&self,
> broker: AnyBroker) -> Option<Arc<dyn PairingAdapter>>` trait
> method (default `None`). Update `SubprocessNexoPlugin` to
> return None (its subprocess handles pairing via existing
> mechanisms). In-tree library impls
> (`nexo_plugin_whatsapp::WhatsappPairingAdapter`,
> `nexo_plugin_telegram::TelegramPairingAdapter`) need a small
> shim that wraps the existing adapter. Boot + spawner replace
> the 6 hardcoded `pairing_registry.register(Arc::new(X::new(broker)))`
> calls with a single loop iterating plugin_handles +
> calling `build_pairing_adapter`. Sites in main.rs: 6318, 7104,
> 8029 + matching whatsapp sites. Bonus: drop `nexo-plugin-whatsapp`
> + `nexo-plugin-telegram` from daemon Cargo.toml IF pairing was
> the last library usage (verify with `grep nexo_plugin_X`). Test:
> manual e2e — telegram pairing flow still completes via wizard.

Telegram inventory snapshot (2026-05-14) — items remaining for
zero-deps:

| # | Category | Where | Blocked on | Effort |
|---|----------|-------|------------|--------|
| A | `register_telegram_tools` outbound | main.rs:5070, 6860 | auth-model generic refactor | multi-week |
| B | `TelegramPairingAdapter::new(broker)` | main.rs:6318, 7104, 8029 | trait method | 1 session — **this is the next milestone** |
| C | `nexo-plugin-telegram` Cargo dep | Cargo.toml:413 | A + B done | trivial |
| D | Setup wizard `if svc.id == "telegram"` | crates/setup/src/lib.rs:641, channels_dashboard.rs:240 | manifest `setup_hint` | 1-2 sessions |
| E | Multi-instance factory hardcode | main.rs:3351-3382 | generic subprocess factory | 1 session |
| F | Config schema `cfg.plugins.telegram` typed | crates/config/* | opaque per-plugin config | multi-week |
| G | Credential store `bundle.stores.telegram` | main.rs:2099, crates/auth/* | auth model refactor | multi-week |
| H | `telegram.yaml` reader | main.rs:9069 | bundled with F | bundled |

### Phase 81.33 deferred — plugin self-discovery (broader than initially scoped)

Spawned 2026-05-14. Phase 81.33.a foundation shipped (steps 1-4 in
plan-runtime-plugin-self-discovery-81.33.a.md):

- ✅ `[[plugin.tools.outbound]]` manifest section
  (`OutboundToolSpec` + validator + 3 tests)
- ✅ `crates/core/src/agent/generic_rpc_tool.rs` —
  `GenericRpcToolHandler` + `map_rpc_error` + 2 unit tests
- ✅ `SubprocessNexoPlugin::register_outbound_tools` trait
  override + `invoke_outbound_tool` helper
- ✅ `src/main.rs` boot + spawner generic loop (additive — old
  hardcoded `register_*_tools` calls coexist during migration)

DEFERRED (single-session budget exhausted during plan exec):

- **Phase 81.33.a step 5 — telegram plugin migration**
  Inspection of `nexo-rs-plugin-telegram/src/tool.rs:43-110`
  surfaced that `publish_outbound` is NOT a simple "publish to
  topic" — it includes credential resolution
  (`nexo_auth::handle::TELEGRAM`), per-agent circuit breaker
  (`ctx.breakers.for_handle(h)`), audit logging
  (`nexo_auth::audit::audit_outbound`), and per-account topic
  suffix derivation. A generic daemon-side `GenericBrokerOutboundTool`
  to replicate this would need manifest fields for credential
  handle type, audit channel, breaker policy, topic template —
  effectively rebuilding the auth model around manifest
  declarations. Realistic scope: 1-2 dedicated phases of design
  + implementation. Brainstorm + spec + plan need a refresh
  pass with this complexity in mind.

- **Phase 81.33.b/.c/.d** — pairing adapter, pairing state,
  setup wizard — same as the original brainstorm but pending
  the auth-model-aware refactor of .a to land first.

- **Daemon Cargo.toml plugin deps** — `nexo-plugin-whatsapp`,
  `nexo-plugin-telegram`, `nexo-plugin-email` still imported as
  library deps. Removal blocked on .a step 5 publishing.

The generic loop in `src/main.rs` is harmless today (registers
zero outbound tools per plugin because no plugin has manifest
entries yet) but provides the integration point for once the
broader refactor lands. Hot-spawned + boot agents already see
the same loop, so when plugins ship manifest declarations, both
paths pick them up automatically.

### Phase 81.32 deferred — runtime hot-spawn

Spawned 2026-05-14. Phase 81.32 c1-c10 shipped: spawn.rs
helpers (`resolve_llm_client`, `validate_agent_config`,
`resolve_workspace_dir`, `assemble_agent_runtime`,
`RuntimeAssemblyDeps`), `AgentSpawnerFn` newtype, `Arc<DashMap>`
for `tools_per_agent` + `agent_snapshot_handles`,
`ConfigReloadCoordinator::set_spawner` + unknown-id branch
(c8) + removed-id branch with `ReloadCommand::Shutdown`
dispatch (c9), spawner closure body in `src/main.rs` running
in **minimal mode** (DelegationTool only), 4 e2e tests in
`crates/core/tests/hot_spawn_test.rs`.

End-to-end: wizard-created agents now activate without restart;
hot-removed agents trigger structured shutdown + firehose
events (`events.runtime.agent.spawned` / `…agent.removed`).

- **Phase 81.32 c7.c — full-parity per-agent tools registry**
  Closure body registers only `DelegationTool` today, then logs
  a WARN about parity. The ~865 LOC boot-loop tools-build span
  (`src/main.rs:4933-5797`, 45 `tools.register` sites — memory
  tool, browser warning, whatsapp/telegram/email/google
  outbound, pollers control, heartbeat, email follow-up flow,
  extension tools, MCP tools) must be lifted into a shared
  `build_per_agent_tools(cfg, ctx) -> Arc<ToolRegistry>` helper
  callable from both boot loop and closure. Subprocess plugins
  still seed per-binding tools via `ScopedToolRegistry` on first
  message so the gap is felt only on direct LLM tool catalog
  introspection, not on outbound channel delivery.
- **Phase 81.32 c7.b.followup — broker firehose handle ordering**
  `ConfigReloadCoordinator` publishes
  `events.runtime.agent.{spawned,removed}` only when its broker
  handle is wired via `start()`. Verify `src/main.rs` reaches
  `coord.start(broker, ...)` BEFORE any wizard hot-spawn can
  fire; current ordering registers the spawner then starts the
  coord later in the same boot scope, so the very first
  hot-spawn after boot may publish to a `None` broker. Move
  `set_spawner` to AFTER `coord.start()` once the rest of c7
  stabilises.
- **Phase 81.32 c11 — operator runbook entry**
  Document the hot-spawn flow + minimal-mode limitation in
  `docs/src/admin/agents-lifecycle.md` (NEW). Reference
  `events.runtime.agent.spawned/removed` topics for dashboard
  subscribers.
- **Phase 81.32 deferred — `runtimes` Vec drainage on hot-remove**
  Boot pushes each `AgentRuntime` into a process-wide `Vec`
  never drained. Hot-removed agents leak a vec entry (the
  runtime task itself stops via `Shutdown`, but the
  `AgentRuntime` struct stays referenced). Convert to
  `Arc<DashMap<String, AgentRuntime>>` and remove on unregister.
- **Phase 81.32 deferred — hot-spawn capacity guard**
  No rate limit on spawner invocation. A malicious admin RPC
  caller could thrash agents.yaml to exhaust broker subscription
  slots. Add a coord-level token bucket (e.g. 10 spawns / min)
  before exposing the wizard outside trusted contexts.

### Phase 81.31 deferred — persona multi-locale

Spawned 2026-05-14 alongside Phase 81.31 shipping. Items observed
during implementation that didn't block the ship but should land
to round out the multi-locale persona surface. Wave 1 of the
follow-up cleanup landed 2026-05-14 (same session): items #1,
#4, #5, #6, #7 shipped; #2 + #3 stay open.

- ✅ ~~**PersonaEditor (Agents page) doesn't expose locale switching**~~
  `nexo-rs-plugin-admin/frontend/src/modules/agents/workspace/PersonaEditor.tsx`
  rewritten with locale dropdown at top + 5 textareas
  (system_prompt + IDENTITY + SOUL + USER + AGENTS) backed by
  `agents/get` + `persona/save_localized`. Dirty-state banner
  warns on locale switch with unsaved edits.
- ✅ ~~**Voice picker integration not wired in wizard**~~ —
  `default_voice_for_locale` relocated to `nexo-tool-meta`
  (Phase 81.31 follow-up #2). `PersonaLocaleEntry` gains
  `recommended_voice: Option<String>` populated per-locale.
  Wizard + PersonaEditor render a "voz recomendada" badge under
  the locale dropdown; operator-override still happens via the
  per-conversation voice mode tool (out of scope for v1).
- ✅ ~~**Atomic rollback for partial `save_localized` failures
  is best-effort**~~ — `FilesystemPersonaStore::save_localized`
  now deletes the four workspace files (`fs::remove_file` per
  path returned by `write_persona_snapshot`) when the yaml patch
  fails. Rollback errors log at warn but don't abort the
  rollback halfway; the typed error includes "rolled back N
  workspace file(s)" so the operator sees what happened.
  Integration test forces the failure via read-only parent dir.
- ✅ ~~**`scripts/regen-ts-types.sh` still emits only to
  `agent-creator-microapp`**~~ — script now fans out to both
  `agent-creator-microapp/frontend/src/api/types.gen.ts` AND
  `nexo-rs-plugin-admin/frontend/src/api/types.gen.ts` by
  default. Override via `TS_REGEN_TARGETS=path1,path2`. Missing
  parents emit a warning but do not abort.
- ✅ ~~**Cody persona repo has the EN system_prompt mirrored
  twice**~~ — v0.2.2 drops the top-level `system_prompt`; the
  agent relies exclusively on `locale_prompts: {en, es}`.
  `min_nexo_version` bumped to `>=0.1.7` (Phase 81.31 ships in
  the next daemon release).
- ✅ ~~**`agents/upsert` does not yet accept `locale_prompts` in
  its input**~~ — `AgentUpsertInput.locale_prompts:
  Option<BTreeMap<String, String>>` added; `upsert_yaml` writes
  the full map (or clears it on `Some({})`). Wizard can now make
  a single round-trip in a follow-up frontend change.
- ✅ ~~**No e2e integration test for `nexo/admin/persona/*`**~~
  `crates/setup/tests/persona_e2e.rs` covers 4 scenarios
  through the real dispatcher (save+get round-trip, missing
  store domain, invalid locale → invalid_params, `agents/upsert`
  with `locale_prompts`).

### Phase 81.30 deferred — plugin-driven pairing UI

Wave 1 of the 81.30 follow-up cleanup landed 2026-05-14 (same
session as 81.31 wave-1): items #2, #5, #6, plus the shared regen
script update (#1) shipped; items #3 + #4 stay open as deferred-
on-purpose.

- ✅ ~~**`scripts/regen-ts-types.sh` parameterize**~~ — shipped
  alongside 81.31 wave-1. Now fans out to microapp + admin plugin.
- ✅ ~~**`nexo-plugin.toml` mirrored in dev-admin.sh heredocs**~~
  `scripts/dev-admin.sh` rewritten to copy each plugin's
  `nexo-plugin.toml` directly from its repo and rewrite ONLY
  `[plugin.entrypoint] command = ...` to point at the dev-state
  binary. Pairing descriptor, extends, capabilities, instructions
  all stay consistent with whatever the binary's `include_str!`
  ships.
- ✅ ~~**`kind = "custom"` flow lacks a real consumer**~~ —
  `PairingModal::CustomFlow` now drives the full
  `pairing/start` + SSE/polling pipeline (same wire as QR) so a
  future plugin declaring `kind = "custom"` just registers a
  `PairingChannelTrigger` and the modal reflects state
  transitions out of the box. Protocol contract documented in
  the component header.
- ✅ ~~**`PairingFormFlow` hardcodes `"instance"` as the credential
  instance discriminator**~~ —
  `PairingSection::instance_field: Option<String>` added (wired
  to `PairingChannelInfo.instance_field`). Telegram manifest
  declares it explicitly; WhatsApp keeps the default. Frontend
  reads from descriptor instead of the literal `"instance"`.
- ✅ ~~**`[plugin.pairing]` validator coverage gap**~~ — Phase 81.30
  follow-up #5: `crates/plugin-manifest/src/validate.rs` now
  rejects `kind = "form"` without fields, `kind = "custom"`
  without `rpc_namespace`, and fields-on-non-form-kind at boot.
  Three new `ManifestError` variants + 7 unit tests.
- ✅ ~~**No e2e integration test for `nexo/admin/pairing/channels`**~~
  `crates/setup/tests/pairing_channels_e2e.rs` covers 4 scenarios
  through the real dispatcher (catalog round-trip, locale
  fallback pt-BR → en, plugins without `[plugin.pairing]`
  filtered out, missing reader → domain not configured).
...
- **31.12.b — wire the Rust SDK into the conformance kit ⬜** —
  `nexo-plugin-sdks/conformance/` (the cross-language kit, shipped
  31.12 in `nexo-plugin-sdks#3`) covers Python/TS/PHP. Add the Rust
  fixture in `nexo-rs` (`crates/conformance-fixture` bin, or
  `crates/microapp-sdk/examples/conformance-fixture.rs --features
  plugin`) implementing the §3.5 `fixture_config` vocab on
  `nexo_microapp_sdk::plugin::PluginAdapter` + `--config <json>` /
  `--print-capabilities`, and a `conformance` CI job in `nexo-rs`
  that builds it, `git clone --depth 1`s `nexo-plugin-sdks`, runs
  `python conformance/run.py --lang rust --fixture <bin>
  --check-contract-version docs/src/plugins/contract.md`. **Reconcile
  first** the 2 divergences the kit surfaced (the Rust child SDK is
  behind the scripting SDKs + contract §5.t here): (1)
  `ToolInvocationError::{ArgumentInvalid,Unavailable}(String)` carry
  no `data` → no `error.data.details` / `error.data.retry_after_ms`;
  (2) `nexo_broker::Event` serializes with extra `id` / `timestamp` /
  `session_id` fields the scripting `Event` wire shape omits — which
  also raises whether the scripting SDKs' `broker.publish` `event`
  shape is what the daemon's `subprocess.rs` actually expects (verify).
  Either loosen the affected scenarios (`$extra_keys: allow` on
  `params.event`; split scenario 14 into code-only + a
  `requires:["error-data"]` data-payload scenario the Rust fixture
  skips; add `"error-data"` to the 3 scripting fixtures' caps) OR fix
  the Rust SDK (bigger — `crates/microapp-sdk` + a `nexo-microapp-sdk`
  republish). Plan §"Step 9" in `plan-31.12-scripting-sdk-conformance-suite.md`.

- **SDK completeness audit (post-extraction) ⬜** — DONE for the
  v1.10.0 surface (31.4.c / 31.8 / 31.9 / 31.10 + the 31.12 kit
  proves cross-SDK parity on it). The remaining audit work is the
  per-surface roadmap 31.13–31.17 (channels / `llm.chat` / hooks /
  memory-vector / `[plugin.extends]` handshake — each needs landing
  in the Rust SDK first) + 31.18 (microapp stdio contract in scripting
  languages — separate track, demand-gated). Tracker:
  `PHASE-31-SDKS-RESUME.md`. ~~Original scope below; superseded.~~
  Once the scripting SDKs are split into their own repo(s), audit each
  for: full `nexo-plugin-contract.md` coverage, methods not yet
  surfaced, whether a microapp-tool-server-side API (the analog of the
  Rust SDK's `ToolCtx` / `ToolReply` / hooks in `crates/microapp-sdk`)
  should exist for Python/TS/PHP at all, parity vs the Rust SDK
  surface, and conformance-test the SDKs against the real extracted
  plugins (browser / telegram / whatsapp / admin) as black-box cases.
...
## Phase 93.5 — typed `cfg.plugins.X` daemon-side reads remaining (Cat B-E)

Phase 93.5.a (PR #55, 2026-05-15) migrated Category A — boot
info logger + presence checks (~10 sites) — onto
`PluginsConfig.entries`. The remaining 20 typed reads in
`src/main.rs` split into four categories, each gated on its
own trigger:

~~**Cat B (6 sites) — credential gauntlet pass-through.**~~
**Shipped 2026-05-15 (PR #56).** `wire::build_credentials`
signature accepts `&PluginsConfig` now; callers stop
threading per-channel slices. Gauntlet still uses typed
access internally for the cross-channel
path/permission/secret checks (`secrecy::SecretString`-bound).

~~**Cat C (~6 sites) — legacy in-process plugin factory clones.**~~
**Audited 2026-05-15, closed as DECLINED.** Hypothesis was
"subprocess-flip left dead code"; reality is each remaining
`cfg.plugins.X.clone()` site has a live consumer (browser
env seeding, wa_pairing state map, subprocess factory
registration, agent-binding validation, autonomous_worker
EmailPlugin). Zero post-flip zombies. See PHASES 93.5.c
for the full audit. Removing further sites blocks on Cat
D/E daemon-orchestration generalisation triggers.

**Cat D (~4 sites) — WhatsApp instance loop.** DEFERRED-strict.
`for (idx, wa_cfg) in cfg.plugins.whatsapp.clone().into_iter().enumerate()`
drives the per-instance pairing state map + public_tunnel
orchestration + session_dir wiring. Daemon-owned native
channel; typed access legitimate. Coupling reach is
daemon-binary local — `WhatsappPublicTunnelConfig` +
`SharedPairingState` are not imported by any `crates/core/`
consumer, so there is NO framework→plugin leak today.
Audit confirmed by 93.5.c (2026-05-15).
**Trigger (strict, all three required):** SECOND channel with
(a) pairing handshake daemon-side AND (b) tunnel side-car
daemon-owned AND (c) HTTP admin RPC for pairing-state
inspect. Sin los tres, generalising is speculative trait
design (CLAUDE.md "Don't design for hypothetical future
requirements"). **Effort when triggered:** ~5h
(`PairingChannelAdapter` trait + WhatsApp impl + main.rs
loop over `cfg.plugins.entries`).

~~**Cat E (~4 sites) — Telegram admin / discovery checks.**~~
**Shipped 2026-05-15 (PR #59).** Validator API (5 public
`validate_*` fns + `spawn::validate_agent_config`) flipped
to `&PluginsConfig`. Internal body uses
`PluginsConfig::instances_for("telegram")` to read declared
instances from the opaque `entries` map. Behavior
unchanged (telegram-only validation); data source now
generic. Spawn closure captures `Arc<PluginsConfig>`
instead of `Vec<TelegramPluginConfig>`.

**Status:** Cat B/C/D/E deferred; each gates on its own
concrete driver. Cat A landed because the trigger (new
plugin auto-surfacing in `nexo doctor`) was real today.

## Phase 93.9 — `nexo_auth::handle::*` channel const-strings retained

Phase 93.9.f closed with `WHATSAPP / TELEGRAM / GOOGLE / EMAIL`
const-strings kept as `pub const` channel-id contract. They
serve as the `DashMap<&'static str, ...>` keys in
`bundle.stores_v2` and as channel-id literals across the
resolver + wire layers. Dropping them would invent a parallel
intern surface for the same set of strings — no real
motivation. Reserved as documentation-only constants.
**Trigger:** if a plugin demands a channel id outside the four
hardcoded ones AND we want consistent channel-id casing
elsewhere. **Effort:** ~1h. **Status:** deferred, low
priority.

## Phase 93.9 — wizard `Arc<GoogleCredentialStore::empty()>` placeholder

`crates/setup/src/services/email.rs:515` passes an empty
`Arc<GoogleCredentialStore>` to `ImapConnection::connect(...)`
+ `SmtpClient::build(...)` because those APIs require a store
even when the auth method is `Password` or `OAuth2Static`
(both skip Google entirely). The wizard's typed import is
legitimate within the wizard process — it never accesses
`bundle.stores`. Real fix: change `ImapConnection::connect` +
`SmtpClient::build` to take `Option<Arc<GoogleCredentialStore>>`
so the wizard can pass `None`. **Trigger:** when refactoring
the email plugin's connection bootstrap, OR if extracting a
standalone email-secrets crate. **Effort:** ~2h.
**Status:** deferred, optional.

~~## Phase 93.10 — channels_dashboard genericisation~~

**Shipped 2026-05-15.** `ChannelDashboardSource` trait + 3
canonical impls (Telegram/Whatsapp/Email) in
`crates/setup/src/services/channels_dashboard.rs`. Whatsapp
impl + helper `whatsapp_auth_state` cfg-gated behind
`plugin-whatsapp`. New `default_dashboard_sources()` registry
+ `detect_channels_with_sources(...)` extension point.
`detect_channels` keeps its public signature (calls registry
internally). `run_link_flow` derives `kinds` list from
sources so slim daemon (`--no-default-features`) hides
whatsapp from picker. Triple win: hardcoded array gone,
slim daemon dashboard reflects features compiled, 5th
channel future = new impl without touching dispatcher.
Validated: `cargo nextest workspace` 6275/6276 +
`-p nexo-rs --no-default-features` 105/105 + `-p nexo-setup
--no-default-features` 317/317.

## ~~Phase 93.10 — original deferred entry~~

Audit 2026-05-15 surfaced `crates/setup/src/services/channels_dashboard.rs`
as a self-describing-plugin violation. The setup wizard's
channel dashboard hardcodes the canonical channel list and
branches on channel id:

- L235: `let kinds = ["telegram", "whatsapp", "email"];`
- L240: `if channel == "telegram" { ... }` (telegram-specific UI branch)
- L69/98/111/119: `ChannelEntry` construction with channel-id
  string literals.
- `crates/setup/src/lib.rs:641` — `if svc.id == "telegram" &&
  should_offer_telegram_link()`.

Consequence: a non-canon plugin (slack/discord/sms) declared
in `plugins.entries` does NOT surface in the setup wizard's
dashboard. The opaque `entries` path stops at config; UI
discovery is still gated on hardcoded knowledge.

**Fix shape:** iterate `cfg.plugins.plugin_ids()` and delegate
metadata to `NexoPlugin::dashboard_descriptor() -> Option<
ChannelDashboardDescriptor>` (new opt-in trait method, default
`None`). Plugins that want dashboard surface implement it;
others stay invisible without breaking the wizard.
**Trigger:** any non-canon plugin demands setup-wizard
visibility, OR Phase 83+ microapp needs to list available
channels generically. **Effort:** ~3h (trait method + 4
canonical-plugin impls + dashboard loop rewrite + test).
**Status:** deferred.

~~## Phase 93.11 — compile-time plugin decoupling audit~~

**Shipped 2026-05-15** as memo
`docs/src/architecture/phase-93-decoupling-audit.md`.
Inventory: 43 code imports + 4 Cargo deps. Classified into
4 buckets: A=tools-registry (9 sites, dies after 81.32.c7.c),
B=pairing-adapter (2 sites, dies after 81.33.b.real),
C=whatsapp-orchestration (12 sites, dies after 93.5.d),
D=email-in-process (15 sites, structurally by design).
**Decision: Hybrid Option 3** — ship Cargo feature-gates for
whatsapp + telegram + browser (~6h, new Phase 93.12); defer
full dynamic-loading until 3rd-party plugin demand or email
subprocess driver. Email stays default-on (no driver to flip).

~~## Phase 93.12 — Cargo feature-gates for canonical plugins~~

Split into 93.12.a (telegram) + 93.12.b (browser placeholder)
+ 93.12.c (whatsapp deferred). 93.11 memo's flat ~6h estimate
turned out to underweight the whatsapp setup-crate
entanglement (admin-RPC dispatcher hardcodes
`with_wa_bot_handle`, `dispatch::TOPIC_OUTBOUND`,
`session::pair_once`). Realistic split:

### Phase 93.12.a — telegram feature-gate ✅ shipped 2026-05-15

`Cargo.toml` gains `[features]` with `default = ["plugin-telegram"]`
+ `plugin-telegram = ["dep:nexo-plugin-telegram"]`.
`nexo-plugin-telegram` switched to `optional = true`.
3 import sites in `src/main.rs` wrapped:
- L1657-1659 — `TelegramPairingAdapter::new(broker)` in
  `build_known_pairing_registry`.
- L5132-5134 — agent-boot fallback
  `register_telegram_tools(&tools)`.
- L6921-6923 — hot-spawn fallback
  `register_telegram_tools(&tools)`.

Validated:
- `cargo build --bin nexo` (default) → `nexo-plugin-telegram`
  in dep graph (`cargo tree -i` confirms).
- `cargo build --bin nexo --no-default-features` → crate
  dropped (`cargo tree --no-default-features -i` errors out:
  "package ID specification ... did not match any packages").
- `cargo nextest run --workspace` → 6276/6276 green
  (default).
- `cargo nextest run --no-default-features -p nexo-rs` →
  105/105 green (slim).

Docs: `docs/src/ops/slim-daemon-build.md` (new, indexed in
SUMMARY.md), mdbook build clean.

### Phase 93.12.b — browser feature-gate ✅ placeholder shipped 2026-05-15

`plugin-browser = []` no-op feature declared in `Cargo.toml`
for symmetry. Browser has no Cargo dep on the daemon
(`seed_browser_subprocess_env` translates typed
`nexo_config::BrowserConfig` into env vars consumed by the
subprocess) so there is nothing to gate today. The flag
exists as a contract anchor — future browser-side helpers
can attach without a churn-PR.

### ~~Phase 93.12.c — whatsapp feature-gate~~ split

Original ~12-15h estimate confirmed during execution.
Split into c.1 (setup scaffolding, shipped) + c.2 (daemon
main.rs orchestration, deferred).

### Phase 93.12.c.1 — whatsapp feature-gate setup scaffolding ✅ shipped 2026-05-15

- `Cargo.toml`: `plugin-whatsapp = ["dep:nexo-plugin-whatsapp",
  "nexo-setup/plugin-whatsapp"]`. Default = [plugin-telegram,
  plugin-whatsapp].
- `nexo-plugin-whatsapp` switched to `optional = true` in
  workspace deps + daemon deps.
- `crates/setup/Cargo.toml`: new `plugin-whatsapp` feature.
- 3 setup-side import sites cfg-gated:
  - `writer.rs:789` — `session::pair_once`. The fn +
    `agent_whatsapp_session_dir` + `set_active_whatsapp_session` +
    `resolve_docker_path` + the match arm in `pair_channel` +
    the dual-shape `wipe_channel_session` all gated.
  - `admin_bootstrap.rs:713` — `with_wa_bot_handle` call wrapped
    in cfg-block; dispatcher chain re-arranged so the call is
    isolated. `WhatsAppTranslator` registration at L771 also
    gated.
  - `admin_adapters.rs:3717-3775` — `WhatsAppTranslator` struct
    + impl + tests module all gated.
- `tests/channel_outbound_end_to_end.rs` — `#![cfg(feature =
  "plugin-whatsapp")]` file-level gate (test exercises
  translator directly).

Validated:
- `cargo check -p nexo-setup --no-default-features` ✅
- `cargo check -p nexo-setup --features plugin-whatsapp` ✅
- `cargo build --release-fast --bin nexo` (default with both
  features) — 1m12s ✅
- `cargo nextest run --workspace` — 6276/6276 green ✅
- `cargo nextest run -p nexo-setup` — 317/317 green ✅
- mdbook clean.

### ~~Phase 93.12.c.2 — daemon main.rs whatsapp orchestration gates~~ ✅ shipped 2026-05-15

All 10 main.rs blocks are now cfg-gated; daemon binary
compiles without `nexo-plugin-whatsapp` linked. Validated
with `cargo build --release-fast --bin nexo
--no-default-features` (3m25s clean) +
`cargo tree --no-default-features -i nexo-plugin-whatsapp`
("did not match any packages") +
`cargo nextest --no-default-features -p nexo-rs` 105/105 +
`cargo nextest --workspace` (default) 6276/6276 +
`cargo nextest --no-default-features -p nexo-setup` 317/317.

Sites gated:

- L709-710 — `RuntimeHealth.wa_pairing: BTreeMap<String,
  nexo_plugin_whatsapp::pairing::SharedPairingState>`
- L1024-1140 — `spawn_whatsapp_pairing_state_subscriber`
  function (uses `SharedPairingState` + `QrSnapshot` types)
- L1654 — `WhatsappPairingAdapter::new(broker)` in
  `build_known_pairing_registry`
- L2493-2495 — `WhatsappPairingTrigger::from_configs` +
  `CHANNEL_ID` in pairing trigger registry
- L3225-3250 — instance loop populating `wa_pairing` map +
  extracting tunnel cfg
- L3500-3539 — `register_instance_subprocess_factories("whatsapp", ...)`
- L3595-3635 — whatsapp pairing subscriber spawn block
- L3792 — `wa_pairing: wa_pairing.clone()` in RuntimeHealth
  construction
- L3804+ — `wa_first_pairing` + tunnel orchestration
- L5128 / L6918 — `register_whatsapp_tools(&tools)` (boot +
  hot-spawn fallbacks)
- L15167-15197 — HTTP `/whatsapp/*` route handler using
  `dispatch_route + WhatsappRoute + PAIR_PAGE_HTML`

The `RuntimeHealth.wa_pairing` field was the spine — gating
it forced gates on every reader (subscriber, instance loop,
HTTP route, tunnel orchestration). Actual effort ~3.5h,
faster than the 6-8h estimate because the readers were
already clustered in identifiable blocks.

Runtime impact when `--no-default-features`: see
`docs/src/ops/slim-daemon-build.md` "whatsapp gate" section.
Status: **shipped**.

## Phase 81.33.b.real reference plugin demo ✅ shipped 2026-05-15

Companion artefact to Stages 1+2+4+5+6 — a working reference
that exercises every manifest section + broker handler topic in
one place. Lives in `crates/test-fixtures/reference-plugin/`.

**Shipped.**

- `nexo-plugin.toml` exercising all 5 manifest sections:
  `[plugin.pairing.adapter]`, `[plugin.http]`, `[plugin.admin]`,
  `[plugin.metrics]`, `[plugin.dashboard]`.
- `src/lib.rs` pure-function handlers per broker topic family:
  - `pairing::normalize_sender`, `send_reply`, `send_qr_image`
  - `http::handle_request` (GET /hello, POST /echo, 404 fallback)
  - `admin::handle` (list, ping, unknown-method error)
  - `metrics::scrape` (Prometheus text with namespaced counters
    + ready gauge)
- 14 unit tests covering each handler's success + edge paths.
- 6 integration tests parsing the real `nexo-plugin.toml`
  against the production `nexo-plugin-manifest` parser; locks
  down the schema contract so a future bump catches it before
  plugin authors copy a broken template.

**Purpose.** Operators / plugin authors copy the crate as a
starting template. Daemon-side stages (1+2+4+5+6) tested only
the failure paths (no subscriber → broker error); the reference
plugin proves the SUCCESS paths in isolation, demonstrating the
JSON wire shape for every contract.

**Why pure functions.** Handlers take `&serde_json::Value` →
`serde_json::Value`. No broker, no tokio runtime, no I/O. Unit-
testable without spinning up a real subprocess. Real plugins
wrap them in a `broker.subscribe(...).await?.for_each(...)`
loop (sketch in the module docstring).

**Validation.**

- `cargo nextest run -p nexo-reference-plugin` — 20/20 green.
- `cargo nextest run --workspace` — 6354/6354 green.
- `cargo build --release-fast --bin nexo --no-default-features`
  — clean (reference plugin not in slim daemon build path).

## Phase 81.33.b.real Stage 6 — manifest-driven dashboard surface ✅ shipped 2026-05-15

Layer 10 polish of the plugin auto-discovery design memo.
Plugins declare `[plugin.dashboard]` with `layout` (Single /
WorkspaceWalk) + `auth_check` (FilePresence / SessionDirFiles)
enums. A generic `ManifestDashboardSource` in `nexo-setup`
interprets the section.

**Shipped.**

- Manifest section in `nexo-plugin-manifest::dashboard`:
  `PluginDashboardSection { layout, auth_check }`. Layout +
  auth_check use `#[serde(tag = "kind")]` enums so TOML stays
  readable. Validation rejects empty/absolute paths, slashes
  in subdir, empty candidates list, etc.
- `ManifestDashboardSource` impl in
  `crates/setup/src/services/channels_dashboard.rs`. Generic
  discovery for Single + WorkspaceWalk layouts; auth probe via
  `probe_auth(secrets_dir, AuthCheck, instance_dir)`. Process-
  wide intern table for `'static` channel_id strings (bounded
  by plugin count).
- `dashboard_sources_from_manifests(&[PluginManifest])` helper
  filters manifests to those declaring the section + builds
  one source per plugin.
- Legacy 3 hardcoded canonical sources
  (`TelegramDashboardSource`, `WhatsappDashboardSource` [cfg-
  gated by `plugin-whatsapp`], `EmailDashboardSource`) stay
  intact. Callers can combine
  `default_dashboard_sources()` + `dashboard_sources_from_manifests(...)`.
...
- `cargo nextest run --workspace` — 6334/6334 (13 new tests:
  8 manifest schema + 5 generic source dispatch).
- `mdbook build docs` clean.

**Migration status.** Canonical plugin crates have not yet
shipped manifest revisions adding `[plugin.dashboard]`. Wizard
keeps using the legacy 3 hardcoded sources. When canonical
plugins migrate, the wizard caller (whoever discovers manifests
+ calls `dashboard_sources_from_manifests`) wires both. Stage 7
cleanup retires the hardcoded sources after all 3 canonical
plugins migrate.

**Operator docs.** `docs/src/plugins/manifest-dashboard.md`
covers the 2 canonical shapes (telegram-like / whatsapp-like)
+ schema reference + migration path + 5 trade-offs.

## Phase 81.33.b.real Stage 5 — plugin Prometheus metrics scrape ✅ shipped 2026-05-15

Layer 11 of the plugin auto-discovery design memo. Plugins
declare `[plugin.metrics] prometheus = true` +
`broker_topic_prefix = "..."`. The daemon's `/metrics` handler
issues a broker JSON-RPC scrape per declaring plugin on every
HTTP request and concatenates the returned Prometheus text into
the aggregate body.

**Shipped.**

- `[plugin.metrics]` manifest section in
  `nexo-plugin-manifest::metrics` (`prometheus` +
  `broker_topic_prefix` + optional `timeout_seconds`).
- `nexo_pairing::plugin_metrics` module: `PluginMetricsDescriptor`
  + `scrape_all(broker, descriptors)` + `scrape_one(broker, d)`
  + typed `PluginMetricsScrapeError::Broker`. Sequential dispatch
  per descriptor (no `futures` dep added) with per-plugin
  timeout; failures warn-log + contribute empty string so one
  slow plugin can't stall the aggregate response.
- `RuntimeHealth.plugin_metrics: Arc<Vec<PluginMetricsDescriptor>>`
  field; daemon boot builds descriptors from
  `wire.plugin_handles[..].manifest().plugin.metrics`.
- `run_metrics_server` appends `scrape_all` output AFTER the
  existing legacy `nexo_plugin_email::metrics::render_prometheus`
  call. Both paths coexist during the migration window.
...
- `cargo nextest run --workspace` — 6321/6321 (5 new tests in
  `plugin_metrics::tests`).
- `mdbook build docs` clean.

**Migration status.** `nexo-plugin-email` has not yet shipped a
manifest revision with `[plugin.metrics]`. Daemon's legacy
in-process call stays. When email migrates, both paths fire
until a cleanup follow-up retires the legacy.

**Operator docs.** `docs/src/plugins/manifest-metrics.md` covers
the manifest schema + broker dispatch contract + failure
isolation + 5 trade-offs.

## Phase 81.33.b.real Stage 4 — daemon-forwarded admin RPC commands ✅ shipped 2026-05-15

Layer 9 of the plugin auto-discovery design memo. Plugins
declare `[plugin.admin] method_prefix = "nexo/admin/<id>/"`
and the daemon's admin dispatcher forwards matches via broker
JSON-RPC. Replaces the per-plugin
`.with_<plugin>_handle(Arc<dyn XxxHandle>)` typed builder
pattern.

**Shipped.**

- `[plugin.admin]` manifest section in
  `nexo-plugin-manifest::admin` (`method_prefix` +
  `broker_topic_prefix` + optional `timeout_seconds`).
  Validation: prefix must start with `nexo/admin/` AND end
  with `/`; root namespace `nexo/admin/` itself rejected.
- `PluginAdminRouter` + `forward_request` + reserved-prefix
  list (10 entries: `nexo/admin/agents|credentials|pairing|llm|
  channels|tenants|memory|sessions|snapshots|policy/`) in
  `nexo-pairing/src/plugin_admin.rs`. Bidirectional collision
  check rejects subpath AND super-prefix to prevent daemon
  endpoint shadowing.
- Router uses `RwLock<Vec<Route>>` interior mutability so the
  daemon constructs an empty `Arc<PluginAdminRouter>` at
  admin-bootstrap time (BEFORE wire) and populates after
  `wire_plugin_registry` returns. In-flight admin RPC sees
  new routes via the shared Arc.
- `AdminRpcDispatcher::with_plugin_admin_router(router, broker)`
  builder method. `call_handler` checks the router BEFORE
  the typed match arms — generic forward wins over legacy
  `with_wa_bot_handle` if the plugin migrates.
- Capability resolution: plugin-router matches map to
  `channels_crud` (reused so operators already granted
  channel admin can call plugin admin without a fresh
  capability).
- `AdminBootstrapInputs.plugin_admin_router:
  Option<Arc<PluginAdminRouter>>` field threaded through 12
  test sites + main.rs init.
- Daemon main.rs registers each plugin's `[plugin.admin]`
  section AFTER `wire_plugin_registry` returns (loop iterates
  `wire.plugin_handles`). Reserved-prefix collisions warn-log
  + skip.

**Validation.**

- `cargo build --release-fast --bin nexo` 3m10s clean.
- `cargo build --release-fast --bin nexo --no-default-features`
  2m53s clean.
- `cargo nextest run --workspace` 6312/6312 (9 new admin
  router tests + 2 new agent_events_firehose test patches).
- `cargo nextest run -p nexo-pairing` 84/84.
- `cargo nextest run --no-default-features -p nexo-rs` 105/105.
- `cargo nextest run --no-default-features -p nexo-setup`
  317/317.
- `mdbook build docs` clean.

**Migration status.** `nexo-plugin-whatsapp` has NOT yet
shipped a manifest revision adding `[plugin.admin]`. Daemon's
legacy hardcoded `nexo/admin/whatsapp/bot/list` + `bot/send`
match arms (via `with_wa_bot_handle`) continue to serve.
When the plugin ships the section, the router matches first
and the legacy block becomes dead code.

**Operator docs.** `docs/src/plugins/manifest-admin.md`
covers the manifest schema + broker dispatch contract +
reserved prefixes + 5 trade-offs.

## Phase 81.33.b.real Stage 2 — daemon-proxied HTTP routes ✅ shipped 2026-05-15

Layer 8 of the plugin auto-discovery design memo. Plugins
declare `[plugin.http] mount_prefix = "/<prefix>"` and the
daemon mounts a generic proxy that forwards requests via broker
JSON-RPC. Replaces the previous hardcoded `/whatsapp/*` block
pattern (still kept as cfg-gated fallback until canonical
plugins ship the manifest section).

**Shipped.**

- `[plugin.http]` manifest section in `nexo-plugin-manifest::http`
  with `mount_prefix` (required) + `timeout_seconds` (optional).
  Validation rejects empty prefix, missing `/` anchor, query/fragment
  separators. Top-level field on `PluginSection` (compat_v1 maps
  to `None`).
- `PluginHttpRouter` + `forward_request` + `PluginHttpResponse`
  in `nexo-pairing/src/plugin_http.rs`. Longest-prefix-first
  matching, last-write-wins per plugin_id (hot-spawn restart
  replaces). Reserved-prefix list (`/health`, `/metrics`, `/pair`,
  `/admin`, `/.well-known`) rejects registrations colliding
  with daemon-internal paths.
- `RuntimeHealth` gains `http_router: Arc<PluginHttpRouter>`
  field; daemon boot iterates `wire.plugin_handles` and
  registers each plugin's `[plugin.http]` section.
- `handle_health_conn` (`src/main.rs:15281+`) checks router
  BEFORE the legacy hardcoded `/whatsapp/*` block. Match
  forwards via broker, decodes the reply (status + headers +
  body_base64), writes raw bytes back. Broker timeout → 504;
  malformed reply → 502. No match falls through to legacy.
- New `write_http_response_bytes` helper alongside the existing
  string-body variant so binary responses (PNGs, PDFs) round-trip
  cleanly. Status text covers 200/201/204/301/302/4xx/5xx codes.
- New `read_http_request` helper parses full request (method,
  path, query, headers, body bytes up to 16KB) into
  `ParsedHttpRequest`. Legacy `read_http_path` becomes a thin
  wrapper that extracts `.path`.

**Limitations.**

- No streaming responses (SSE / chunked transfer). Plugin must
  buffer full response.
- No WebSocket upgrades. WS endpoints stay on `[plugin.http_server]`
  (plugin binds own port).
- 16KB request body cap.

**Validation.**

- `cargo build --release-fast --bin nexo` 3m11s clean.
- `cargo build --release-fast --bin nexo --no-default-features`
  2m53s clean.
- `cargo nextest run --workspace` 6294/6294 green (8 new
  plugin_http tests).
- `cargo nextest run -p nexo-pairing` 75/75 (11 plugin_http tests
  total: matching logic, reserved-prefix rejection, response
  decoding, broker error path).
- `cargo nextest run --no-default-features -p nexo-rs` 105/105.
- `mdbook build docs` clean (new `plugins/manifest-http.md`).

**Migration.** Canonical plugins (whatsapp, telegram, email)
have not yet shipped manifest revisions adding `[plugin.http]`.
Daemon's legacy cfg-gated `/whatsapp/*` block continues to serve
until they do. When a plugin ships the section, the router
matches first and the legacy block becomes dead code (still
cfg-gated, removable in follow-up).

## ~~Phase 81.33.b.real — manifest-driven pairing-adapter schema~~   ✅ shipped 2026-05-15

Re-attacked after user clarified that auto-discovery is a real
requirement (new plugins must drop in without daemon edits),
not a hypothetical trigger. Shape-risk on `normalize_sender`
resolved via Pattern A from the plugin auto-discovery design
memo (`docs/src/architecture/plugin-auto-discovery.md`):
broker-RPC dispatch with in-memory cache so per-channel custom
logic stays in the plugin subprocess.

**Shipped.**

- `[plugin.pairing.adapter]` manifest section in
  `nexo-plugin-manifest` (`channel_id`, `broker_topic_prefix`,
  optional `format_challenge_text_kind` + `normalize_cache_ttl_seconds`).
- `GenericBrokerPairingAdapter` in `nexo-pairing/src/generic_adapter.rs`
  — dispatches `PairingChannelAdapter` trait methods via broker
  JSON-RPC, caches `(raw → Option<normalized>)` to avoid
  per-call round trips, leaks `channel_id` to `&'static str` at
  construction.
- `SubprocessNexoPlugin::build_pairing_adapter()` returns the
  generic adapter when manifest declares the section; otherwise
  the trait's default `None` keeps subprocess plugins on the
  legacy daemon-side hardcoded path.
- Daemon's primary boot loop (`src/main.rs:6416`) + hot-spawn
  loop (`:7224`) iterate `plugin_handles` and register every
  manifest-driven adapter alongside the legacy registrations.
  `PairingAdapterRegistry::register` overwrites by `channel_id`,
  so a manifest-declared adapter wins over the matching legacy
  hardcoded one.
- Dispatch-ctx mode (`boot_dispatch_ctx_if_enabled`) still uses
  legacy registry only — `plugin_handles_cell` not threaded
  into that helper. Documented as follow-up if dispatch-ctx
  pairing-aware hooks ever materialise.

**Operator-facing docs.** `docs/src/plugins/manifest-pairing-adapter.md`
documents the manifest section + broker dispatch contract + 5
trade-offs.

**Validation.** `cargo build --release-fast --bin nexo` (default,
3m09s), `--no-default-features` (2m50s). `cargo nextest run
--workspace` 6280/6280 (4 new tests in `generic_adapter::tests`
covering `channel_id` leak, cache write-through on broker
failure, cache hit avoids broker, TTL eviction).
`-p nexo-pairing` 67/67. `-p nexo-rs --no-default-features`
105/105. mdbook clean.

**Migration status.** Canonical plugin crates
(`nexo-plugin-whatsapp`, `nexo-plugin-telegram`) have NOT yet
shipped a manifest revision with the section. Daemon's legacy
cfg-gated `Arc::new(<plugin>PairingAdapter::new(broker))` calls
in `build_known_pairing_registry()` continue to serve until the
next plugin release. When a plugin ships the section, the
generic adapter overwrites in the registry without operator
action; the legacy block can then be retired in a follow-up.

## Phase 81.33.b.real legacy entry — original deferred rationale

Re-evaluated 2026-05-15 alongside 93.10 / 93.12.c. Verdict:
same shape-risk as 93.5.d, smaller blast radius. Holding
DEFERRED-strict on a sharp trigger.

**Status today.** The two daemon-side hardcoded
`Arc::new(XxxPairingAdapter::new(broker))` blocks in
`build_known_pairing_registry`
(`src/main.rs:1654-1659` whatsapp, `src/main.rs:1660-1663`
telegram) are **already feature-gated** by Phase 93.12.a
(`#[cfg(feature = "plugin-telegram")]`) + Phase 93.12.c.2
(`#[cfg(feature = "plugin-whatsapp")]`). Slim daemon binary
drops both adapters cleanly. A 3rd canonical channel would
add its own `#[cfg(feature = "plugin-X")]` block — linear
scaling, no compile-time coupling for operators who don't
need the channel.

**What 81.33.b.real would change.** Replace the hardcoded
constructor block with a generic loop:
```rust
...
```
Plus a manifest schema `[plugin.pairing.adapter]` so
out-of-tree subprocess plugins can declare an adapter
without shipping a Rust trait impl linked into the daemon.

**Why DEFERRED-strict, not "next session".**
`PairingChannelAdapter::normalize_sender(raw: &str) -> Option<String>`
is per-channel custom logic that does NOT generalise
cleanly through a manifest:
- whatsapp: strip `@c.us`, ensure E.164 with `+` prefix.
- telegram: lowercase username when starts with `@`;
  numeric chat_id passes through unchanged.
- a hypothetical signal: phone number safety codes.
- a hypothetical sms: carrier-specific E.164 + region.

Expressing this as manifest data requires either (a) a
universal regex (does not exist — the rules are not
pure regex), (b) a hardcoded enum
`kind: whatsapp_jid|telegram_handle|...` (re-introduces
the framework→plugin coupling Phase 93 just eliminated),
or (c) embedded scripting (Lua/Rhai — heavy new dep, runtime
sandboxing surface, slow first-message latency).

Designing the schema with only 2 real consumers (whatsapp +
telegram) is the same shape-risk Phase 93.5.d was held back
for. We need at least 3 sample normalise rules to choose
between regex-table / enum / script directions with
confidence; today we have 2.

**Trigger (strict, all required).**
1. THIRD canonical or 3rd-party plugin ships a
   `PairingChannelAdapter` impl with a real
   `normalize_sender` rule — gives a 3rd sample for shape
   design.
2. AND either (a) the 3rd-party demands out-of-tree
   registration without daemon recompile, OR (b) the Phase
   93.11 audit memo's "dynamic-loading" direction is
   chosen (currently "Option 3 Hybrid: defer dynamic-
   loading").
3. AND the daemon-side hardcoded constructor count is
   still ≤ 5 sites (the cfg-gate scaling stops being
   linear-cheap around then).

Without all three, hardcoded + cfg-gated is the robust
shape. The cfg-gate scaling is good enough for at least
the next two canonical channels.

**Effort when triggered.** ~5h: manifest schema design
informed by 3+ samples + `GenericBrokerPairingAdapter` impl
