...
| 31.1.b — Tarball extraction | **P3** | ⬜ NEW | ~0.5 d. After verified download, untar into `plugins.discovery.search_paths` directory. Reject path traversal entries; preserve executable bits on the binary; validate the extracted manifest matches what `resolve_release` saw. |
| 31.1.c — `Mode::PluginInstall` CLI integration | **P3** | ⬜ NEW | ~0.5 d. New main.rs Mode + parser arm. `nexo plugin install owner/repo[@tag]` chains: parse coords → install_plugin → extract → optionally trigger Phase 18 hot-reload so the plugin loads immediately. Output: human-readable progress + final `plugin.lifecycle.<id>.installed` broker event. |
