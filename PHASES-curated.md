...
| **81.14 — `SubprocessNexoPlugin` adapter (host-side spawn + stdio JSON-RPC bridge)** | **P3** | ✅ shipped 2026-05-01 (manifest `[plugin.entrypoint]` additive section + `SubprocessNexoPlugin` host-side adapter + `subprocess_plugin_factory` helper + 9 unit tests covering happy path of spawn/handshake plus error paths: missing command, env collision with reserved `NEXO_*`, initialize-reply timeout, manifest id mismatch, shutdown idempotency. JSON-RPC 2.0 newline-delimited over stdio mirrors `extensions/openai-whisper` shape. Broker → child topic bridge wires in 81.14 follow-up — this slice only ships spawn + handshake plumbing. Existing 4 in-tree plugin manifests (browser/telegram/whatsapp/email) verified still parse with new optional `entrypoint` section.) |
