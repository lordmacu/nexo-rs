...
| **81.20.b.b — main.rs threads `LlmServices` into subprocess runtime + streaming via `llm.complete.delta` notifications** | **P3** | ⬜ NEW | ~1.5 d. (1) Plumb `cfg.llm: Arc<LlmConfig>` into `SubprocessRuntime.llm` so handler stops returning -32603. Either extend `PluginInitContext` with `llm_config: Arc<LlmConfig>` (so SubprocessNexoPlugin::init builds LlmServices inline from `ctx.llm_registry + ctx.llm_config`) OR thread LlmServices via `SubprocessCtxStubs`. (2) Streaming: switch `client.chat` to `client.stream`, emit `llm.complete.delta` notifications per chunk + final `llm.complete.done` notification with usage; reply to the original request with the same correlation id but result containing only the final usage + finish_reason. SDK-side helpers for streaming consumption land in 81.15.c. |
