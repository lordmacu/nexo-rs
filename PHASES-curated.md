...
| **81.21.b — Plugin supervisor: auto-respawn + backoff** | **P3** | ⬜ DEFERRED | ~1 d. After 81.21 detects crashes, operator gets notified but daemon doesn't auto-recover. 81.21.b adds `manifest.supervisor.{respawn: bool, max_attempts: u32, backoff_ms: u64}` config + respawn loop in supervisor task. Stderr tail capture (~last 32 lines) attached to crash event for context. Required before community-tier plugins are safe in production. |
