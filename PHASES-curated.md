...
| **81.20.a.b — main.rs reorder so `long_term_memory` reaches `SubprocessRuntime`** | **P3** | ⬜ NEW | ~30 LOC. Today main.rs:1977 passes `long_term_memory: None` because the handle is constructed at line 10883 — after the wire callsite at 1984. Reorder construction so memory is available at the wire callsite. Until this lands, subprocess plugins receive -32603 "memory not configured" for every memory.recall request, regardless of whether the operator has long-term memory enabled. |
| **81.20.b — Daemon-mediated RPC: `llm.complete` (with streaming)** | **P3** | ⬜ NEW | ~1.5 d. Extends `handle_child_request` match with `llm.complete`. Params: `{ provider, model, messages, max_tokens?, temperature? }`. Result: `{ content, finish_reason, usage }`. Streaming is harder — needs a separate notification channel (`llm.complete.delta`) since JSON-RPC doesn't support partial responses. CircuitBreaker around the `LlmRegistry::build` + `complete` calls. |
| **81.20.c — Daemon-mediated RPC: `tool.dispatch`** | **P3** | ⬜ NEW | ~1 d. Extends `handle_child_request` match with `tool.dispatch`. Params: `{ tool_name, arguments }`. Result: tool's standard `ToolReply` shape. ToolRegistry needs to be in BridgeContext (currently only in SubprocessCtxStubs as `Arc::new(ToolRegistry::new())` — empty stub). |
