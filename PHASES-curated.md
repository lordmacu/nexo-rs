...
| 31.8 — Operator UI: `nexo plugin list` / `upgrade` / `remove` | **P3** | ✅ shipped 2026-05-04 (new `src/plugin_admin.rs` module + 3 `Mode` variants + 3 parse arms + 3 dispatch arms in `src/main.rs`. Argv: `nexo plugin list [--include-orphan] [--json]`, `nexo plugin upgrade <id> [--target <triple>] [--require-signature\|--skip-signature-verify] [--json]`, `nexo plugin remove <id> [--purge-cache] [--yes] [--json]`. New `<plugin_dir>/.nexo-install.json` schema (`schema_version: "1.0"`) recording `id, version, owner, repo, tag, target, sha256, installed_at (RFC3339), source: "github-releases"`. `plugin_install.rs::run_plugin_install` patched to call `write_install_metadata` after extract success (skipped on `was_already_present`); soft-fail with stderr warn so plugin still works without metadata (treated as orphan by `list` / `upgrade`). `discover_installed_plugins` walks every search_path's immediate child dirs, parses `nexo-plugin.toml` + optional `.nexo-install.json`, sorted by id. `list` filters orphans by default; `--include-orphan` surfaces them with `(orphan)` suffix; JSON shape `PluginListReport { ok, plugins[] }`. `upgrade` reads metadata, builds `PluginCoords { owner, repo, tag }`, calls `resolve_release` against `--target` override or recorded target; emits `PluginUpgradeReport { was_no_op: true }` on same-version, `DowngradeRefused` error on lower-version, otherwise delegates to `run_plugin_install` (single code path for download/verify/extract/cosign — JSON line is the install report). `remove` requires `--yes` (TTY prompt deferred to 31.8.b → `NeedsYesConfirm` error otherwise); atomic via rename-aside `<plugin_dir>.removing-<rand>` + `remove_dir_all`; `--purge-cache` walks `nexo_state_dir/plugins/<id>` + `nexo_state_dir/plugins/cache/<id>`. Best-effort `plugin.lifecycle.<id>.removed` broker emit (NATS only, 2s timeout, mirror install pattern). `AdminError` enum with 9 variants (`PluginNotFound`, `NoInstallMetadata`, `MetadataInvalid`, `ManifestRead`, `ConfigLoad`, `Io`, `DowngradeRefused`, `NeedsYesConfirm`, `Resolve(InstallError)`). 11 unit tests: metadata round-trip + read-none + read-invalid + discover-skips + discover-collects + list-filters + list-includes + list-empty + admin-error-kind exhaustive + aside-path-differs + list-entry-fields. Help text added in `print_plugin_help` + `print_usage`. NO interactive TTY prompt (defer 31.8.b), NO semver constraint pinning beyond tag tracking (defer 31.8.c), NO upgrade-side cosign duplication (delegated). Workspace builds clean; `cargo test --bin nexo plugin` 42/42 (11 admin + 12 install + 8 run + 11 new).) |
| 31.9 — Docs: plugin authoring guide per language + contract reference + signing how-to | **P3** | ✅ shipped 2026-05-04 (4 new/expanded mdbook pages + 1 sync script. New `docs/src/plugins/authoring.md` (~200 LOC) — entry-point overview with "Plugin vs Extension vs Microapp" decision-tree table + 4-language picker table + 5-min Rust quickstart + local dev loop conventions. New `docs/src/plugins/rust-sdk.md` (~250 LOC) — espejo estructural de python/ts/php sdk pages: `PluginAdapter` builder API table, manifest example, scaffold+build+run quickstart, smoke test, per-target tarball convention. New `docs/src/plugins/signing-and-publishing.md` (~300 LOC) — end-to-end tutorial: unsigned `gh release create v0.1.0` → `gh variable set COSIGN_ENABLED --body true` opt-in → operator-side `[[authors]]` block in `trusted_keys.toml` → `nexo plugin install --require-signature` round-trip with sample human + JSON outputs. New `scripts/sync-plugin-contract.sh` (~60 LOC bash) — vendors workspace-root `nexo-plugin-contract.md` into `docs/src/plugins/contract.md` with auto-vendored HTML comment header + "See also" cross-link footer; `--check` mode for CI gate (deferred to 31.9.b). Expanded `docs/src/plugins/contract.md` from 28 LOC stub → 678 LOC vendored copy via initial sync run. `SUMMARY.md` "Plugin SDKs" section reordered: Authoring overview / Plugin contract / Patterns / Rust SDK / Python SDK / TypeScript SDK / PHP SDK / Publishing / Signing & publishing. `mdbook build docs` clean. NO `docs/src/plugin-authoring/` subdir (kept under existing `docs/src/plugins/` to preserve URLs). NO mdbook-include or mdbook-cmdrun deps (sync script is dependency-free). NO Spanish content. NO frontmatter (plain Markdown only). NO duplication of `ops/plugin-trust.md` troubleshooting (cross-linked).) |
