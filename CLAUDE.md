...
**Progress: 159 implemented sub-phases / 5 deferred sub-phases tracked in `PHASES.md` (26.z tunnel.url, 26.aa pair_approve tool [security-review-gated], 26.ab pairing.yaml loader, 19.x pollers V2 backlog, 38.x flakes & races). Phase 67 (Claude Code self-driving agent) — 67.0–67.9 shipped (foundation, spawn, binding, permission, loop, acceptance, git-worktree, memory, replay-policy, compact-policy); 67.10–67.13 backlogged. Phase 68 (Local LLM tier — llama.cpp) — 14 sub-phases backlogged (transversal tier-0 inference for PII / embeddings / poller pre-filter / classifiers / fallback; **model-agnostic GGUF loader** — gemma3, qwen2.5, llama3.2, phi3, smolLM, etc. all swappable per-job via config — defaults gemma3-270m + bge-small via `llama-cpp-2`, target Termux ARM CPU + desktop CPU/GPU). Phase 21 (link understanding), Phase 25 (web_search), and Phase 26 (pairing + per-channel reply adapters) shipped. Phases 22–24 backlogged in `PHASES.md` (Slack/Discord plugins, realtime voice, image generation). `web_fetch` deferred — Phase 21 covers user-shared URLs. Pairing companion-tui deferred and direct in-process `Session::send_text` delivery for the challenge is deferred (adapters currently publish via broker on `plugin.outbound.{channel}`); store + gate + CLI + WA/TG `PairingChannelAdapter` impls + adapter registry all live. Detailed status of every deferred item lives in `proyecto/FOLLOWUPS.md`.**
