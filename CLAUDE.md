...
**Progress: 148 implemented sub-phases / 6 deferred sub-phases tracked in `PHASES.md` (26.y telemetry rest, 26.z tunnel.url, 26.aa pair_approve tool [security-review-gated], 26.ab pairing.yaml loader, 19.x pollers V2 backlog, 38.x nexo-llm flake serial guard). Phase 21 (link understanding), Phase 25 (web_search), and Phase 26 (pairing + per-channel reply adapters) shipped. Phases 22–24 backlogged in `PHASES.md` (Slack/Discord plugins, realtime voice, image generation). `web_fetch` deferred — Phase 21 covers user-shared URLs. Pairing companion-tui deferred and direct in-process `Session::send_text` delivery for the challenge is deferred (adapters currently publish via broker on `plugin.outbound.{channel}`); store + gate + CLI + WA/TG `PairingChannelAdapter` impls + adapter registry all live. Detailed status of every deferred item lives in `proyecto/FOLLOWUPS.md`.**
