...
| 90 | nexo-plugin-admin — official admin plugin out-of-tree | 18/18 | ✅ shipped 2026-05-10 (core: 90.1 scaffold + 90.2 frontend clone + 90.3 9 modules + 90.4 auth/tunnel + 90.5 daemon shim + 90.6 cargo publish + 90.7 admin-ui archive; follow-ups in same wave: 90.x.cleanup -2702 LOC dead admin code + 90.x.tenants CRUD + 90.x.mcp admin RPC + 90.x.plugins doctor admin RPC + 90.x.memory query + 90.x.channels-approve + 90.x.token-rotated cookie swap + 90.x.memory-snapshot list/delete + shared cell + 90.x.changelog + 90.x.github-remote (lordmacu/nexo-rs-plugin-admin public) + 90.x.e2e 3 binary smoke tests; published 9 plugin versions + nexo-tool-meta 0.1.6→0.1.11 + nexo-core 0.1.6→0.1.11; daemon binary now 0 warnings was 45 at session start; ~~daemon-backed e2e test~~ deferred follow-up, ~~memory snapshot create/restore~~ CLI-only deferred) |
