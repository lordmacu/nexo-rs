...
| 79 | Tool surface parity sweep (claude-code-leak tools) | 79.1 EnterPlanMode/ExitPlanMode ✅, 79.2 ToolSearchTool ✅ (MVP — provider filtering deferred), 79.3 SyntheticOutputTool ✅, 79.4 TodoWriteTool ✅, 79.5 LSPTool (rust-analyzer/pylsp/tsserver), 79.6 TeamCreate/TeamDelete (N-parallel coordinated), 79.7 ScheduleCronTool (LLM-time cron), 79.8 RemoteTriggerTool, 79.9 BriefTool (terse-mode toggle), 79.10 ConfigTool (gated self-config), 79.11 McpAuth + ListMcpResources + ReadMcpResource, 79.12 REPLTool (Python/Node stateful sandbox), 79.13 NotebookEditTool (Jupyter cell edits), 79.14 docs + admin-ui sync | 4/14 🔄 |
