...
| 81 | Plug-and-play plugin system | 35/35 | 🔄 (81.3/81.4/81.5/81.6/81.7/81.8/81.9/81.9.b/81.10/81.11/81.12.a-d/81.13 manifest unification/81.14/81.14.b/81.15.a-c.b/81.16/81.17/81.17.b/81.17.c/81.18/81.18.b.1 telegram subprocess flip/81.18.b.2 whatsapp flip + pairing bridge/81.19.a/81.19.b email extract + factory flip/81.20.a-b.c/81.21/81.21.b/81.21.b.b plugin supervisor auto-respawn + real uptime telemetry + manual restart RPC (frontend + SharedPluginHandles cell wired in main.rs)/81.22/81.23/81.24/81.25/81.26/81.27/81.28/81.29 ✅; 81.12.e DEFER → 81.17; 81.20.c typing presence RPC/81.21.c pending) |
| 82 | Multi-tenant SaaS extension enablement | 15/15 | ✅ shipped 2026-05-01 (follow-ups in FOLLOWUPS.md) |
| 83 | Microapp framework foundation | 16/17 | 🔄 (83.3/83.4/83.5/83.6/83.7/83.8/83.11/83.12 + ts-types-codegen + theme/83.13 MVP/83.14 publish-readiness + 83.14.b cargo+npm publish exec/83.15/83.16/83.17 ✅; 83.1/83.2/83.9/83.10 pending — heavy product/UI sub-phases) |
...
| 90 | nexo-plugin-admin — official admin plugin out-of-tree | 19/19 | ✅ shipped 2026-05-10 (core: 90.1 scaffold + 90.2 frontend clone + 90.3 9 modules + 90.4 auth/tunnel + 90.5 daemon shim + 90.6 cargo publish + 90.7 admin-ui archive; follow-ups in same wave: 90.x.cleanup -2702 LOC dead admin code + 90.x.tenants CRUD + 90.x.mcp admin RPC + 90.x.plugins doctor admin RPC + 90.x.memory query + 90.x.channels-approve + 90.x.token-rotated cookie swap + 90.x.memory-snapshot list/delete + shared cell + 90.x.memory-snapshot.create-restore (admin RPC create+restore + dry_run preview + tenant validation + encryption_available flag) + 90.x.changelog + 90.x.github-remote (lordmacu/nexo-rs-plugin-admin public) + 90.x.e2e 3 binary smoke tests; published 10 plugin versions + nexo-tool-meta 0.1.6→0.1.12 + nexo-core 0.1.6→0.1.12; daemon binary now 0 warnings was 45 at session start; ~~daemon-backed e2e test~~ deferred follow-up) |
| 91 | STT pure-Rust migration via Candle | 11/12 | 🔄 (91.1 ✅ API research lock-in / 91.2 ✅ Cargo deps + feature scaffold / 91.3 ✅ mel spectrogram — Candle's `m::audio::pcm_to_mel` reused so rustfft dropped / 91.4 ✅ Candle Whisper inference / 91.5 ✅ tokenizer + token decode rolled into 91.4 / 91.6 ✅ feature dispatch — `stt-candle` wins re-export when both features on / 91.7 ✅ `model_id` field + HF Hub auto-fetch / 91.8 ✅ parity test scaffold (3 fixtures, WER threshold, `#[ignore]` env-var-driven) / 91.9 ✅ cross-compile matrix 5/6 verde — Linux gnu/musl × 2 + Win MSVC + Android NDK r27c with `RUSTFLAGS="+fp16,+fhm"`; WASM blocked by hf-hub → mio (Phase 91.x.wasm follow-up) / 91.10 ✅ `stt-candle-{metal,cuda,accelerate}` opt-in features / 91.11 ✅ docs + platform-support row updated; 91.12 DEFER deprecate `whisper-rs` after 2 release cycles; 91.x.wasm follow-ups: ✅ phase-2 tokio split / ✅ phase-4 cloud trait + OpenAI + Groq + Anthropic stub + CompositeProvider / ✅ phase-4b Anthropic voice_stream WebSocket real (15 tests) / ✅ phase-4d LocalCandleProvider bridge (6 tests, RAII tempfile, Cow lang-hint override) / ✅ phase-4c.2 hand-assembled multipart body (6 tests, RFC 7578) / ✅ phase-4c.3 workspace reqwest pin trim + SDK per-target split + SttProvider Send/Sync split for wasm32 (REST cloud STT now compiles on wasm32-unknown-unknown — 9 workspace crates updated); ⬜ phase-4c.4 voice_stream WASM gloo-net swap-in + phase-4b.streaming live PTT + phase-4b.segmentation + phase-4e transformers.js — deferred) |
