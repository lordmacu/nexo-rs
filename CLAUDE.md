...
| 81 | Plug-and-play plugin system | 35/35 | 🔄 (81.3/81.4/81.5/81.6/81.7/81.8/81.9/81.9.b/81.10/81.11/81.12.a-d/81.13 manifest unification/81.14/81.14.b/81.15.a-c.b/81.16/81.17/81.17.b/81.17.c/81.18/81.18.b.1 telegram subprocess flip/81.18.b.2 whatsapp flip + pairing bridge/81.19.a/81.19.b email extract + factory flip/81.20.a-b.c/81.21/81.21.b/81.21.b.b plugin supervisor auto-respawn + real uptime telemetry + manual restart RPC backend/81.22/81.23/81.24/81.25/81.26/81.27/81.28/81.29 ✅; 81.12.e DEFER → 81.17; 81.20.c typing presence RPC/81.21.c pending; SharedPluginHandles cell wiring deferred for manual restart RPC) |
...
| 91 | STT pure-Rust migration via Candle | 0/12 | ⬜ planned (P3 — POST-CRITICAL; full plan in [`PHASE-91-STT-CANDLE-MIGRATION-PLAN.md`](PHASE-91-STT-CANDLE-MIGRATION-PLAN.md). Replaces `whisper-rs` (whisper.cpp C++ binding) with HuggingFace Candle pure-Rust ML inference; eliminates the VS Build Tools 2022 + CMake requirement on Windows + unblocks trivial Android NDK / WASM cross-compile. Audio decode pipeline unchanged — only the inference layer swaps. 12 sub-phases: 91.1 API research / 91.2 Cargo deps + feature scaffold / 91.3 mel spectrogram (`rustfft`) / 91.4 Candle Whisper inference / 91.5 tokenizer + token decode / 91.6 feature dispatch (`stt-whisper-cpp` legacy vs `stt-candle` default-track) / 91.7 model format + HF Hub auto-fetch / 91.8 parity tests vs whisper-rs (WER < 5%) / 91.9 cross-compile matrix (Linux musl × 2 + macOS + Windows MSVC + Android NDK + WASM) / 91.10 GPU acceleration features (Metal / CUDA / accelerate, opt-in) / 91.11 docs + CHANGELOG / 91.12 DEFER deprecate `whisper-rs` after 2 release cycles) |
