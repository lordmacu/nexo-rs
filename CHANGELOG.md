...
- **Phase 31.5.c — PHP plugin SDK + template (Fibers, robust).**
  ...
  plugins. Two new in-tree directories:
  
  - `extensions/sdk-php/` — Composer package `nexo/plugin-sdk`
    with PSR-4 namespace `Nexo\Plugin\Sdk\`, SPDX license
    `MIT OR Apache-2.0`, explicit `version: "0.1.0"` for
    path-repo resolution. Public API mirrors the other SDKs:
    `PluginAdapter` (Fiber-based scheduler), `BrokerSender`
    (FIFO writes via direct `fwrite(STDOUT, ...)`), `Event`
    (readonly value object), `Manifest::parse` (TOML reader
    with full validation), exception classes (one per file
    per PSR-4), `StdoutGuard` (defensive `ob_start`), `Wire`,
    `Scheduler`. Single runtime dep `yosymfony/toml: ^1.0`.
    PHP `^8.1` minimum (Fibers required).
  - `extensions/template-plugin-php/` — drop-in template with
    manifest, `src/main.php` echo handler, `composer.json`
    declaring SDK via path repository (`url: ../sdk-php`,
    `symlink: false`, `minimum-stability: dev`,
    `prefer-stable: true`), `composer.lock` checked in, helper
    scripts, end-to-end pack test, `.github/workflows/release.yml`.
  
  Robustness defaults all default-on (mirroring Phase 31.5 TS):
  
  - `enableStdoutGuard:true` — `ob_start` callback diverts
    non-JSON `echo`/`print`/`printf`/`var_dump` to stderr
    tagged `[stdout-guard]`. Documented limitation:
    `fwrite(STDOUT, $x)` direct writes BYPASS — SDK's
    BrokerSender uses this deliberately for blessed JSON
    frames. Authors must use `echo`/`print` for stdout (those
    are guarded).
  - `maxFrameBytes:1048576` — rejects oversized inbound
    frames with `WireError`.
  - `handleProcessSignals:true` — `pcntl_async_signals(true)`
    + SIGTERM/SIGINT trigger graceful shutdown.
  - In-flight Fiber drain on `shutdown` request via
    `Scheduler::drain()` — same idiom as Python's
    `_drain_inflight` and TS's `Promise.allSettled`.
  - Single-shot `run()` throws `PluginError` on second call.
  
  The Fiber scheduler (`extensions/sdk-php/src/Scheduler.php`)
  preserves the "reader does not block on slow handler"
  contract invariant proven necessary by the TS + Python tests.
  Each `broker.event` handler runs in its own Fiber; the
  dispatch loop ticks the scheduler between `stream_select`
  polls of stdin so suspended Fibers (e.g. authors who call
  `Fiber::suspend()` between operations) get resumed without
  blocking the reader.
  
  Tarball convention reuses the `noarch` target shipped in
  Phase 31.4 — no resolver changes needed. Layout:
  ...
  Bash launcher uses `php -d display_errors=stderr -d
  log_errors=0` so PHP errors land on stderr (defense-in-depth
  with the stdout guard). Operator host does NOT need Composer
  installed — the `vendor/autoload.php` shipped is plain PHP
  generated by `composer install --no-dev --optimize-autoloader
  --classmap-authoritative` at CI time. Just `php-cli` 8.1+.
  
  Pure-PHP deps constraint: `verify-pure-php.sh` audits
  `lib/vendor/` for `*.so` / `*.dylib` / `*.dll` and fails
  the publish workflow if any are present. Native-ext PHP
  plugins → per-target tarballs (Phase 31.5.c.b, deferred).
  
  Composer integration: templates use a path repository
  (`type: "path"`) with `symlink: false` — without that flag
  Composer creates a symlink in `vendor/nexo/plugin-sdk/`
  pointing at the path repo, which would break when the
  tarball is extracted on the operator host. With
  `symlink: false` Composer copies the files physically.
  `composer.lock` checked in for templates (reproducibility);
  SDK omits the lockfile so consumers resolve fresh.
  `composer validate --strict` runs as a CI gate.
  
  Publish workflow shape (`.github/workflows/release.yml`):
  
  - Same 4-job structure as Phase 31.2/31.4/31.5.
  - Build matrix has a single `noarch` entry.
  - Uses `shivammathur/setup-php@v2` with `php-version: "8.3"`
    + `tools: composer:v2`.
  - Pre-vendor: `composer install --no-dev --optimize-autoloader
    --classmap-authoritative`.
  - Post-pack: `bash scripts/verify-pure-php.sh
    .audit/lib/vendor` as hard gate.
  - Sign + release jobs identical to other templates.
  
  Real handshake smoke verified locally: `echo
  '{"jsonrpc":"2.0","id":1,"method":"initialize"}' | php
  src/main.php` returns the valid JSON-RPC manifest reply
  through the SDK's full pipeline.
  
  14 test cases across 7 test files using a stdlib-style
  `proc_open` runner (no PHPUnit / Pest dep — mirrors TS +
  Python choices):
  
  - **Handshake (3)**: initialize reply, unknown method
    `-32601`, unknown notification silently ignored.
  - **Manifest validation (3)**: missing id raises
    `ManifestError` with `field`, invalid TOML raises,
    id regex violation raises.
  - **Dispatch (3)**: handler invocation; slow handler
    (Fiber suspends ~200ms) does not block reader; in-flight
    Fibers drained on shutdown (proves no mid-publish
    cancellation).
  - **Stdout guard (2)**: idempotent install/uninstall;
    `echo "hello-from-noisy-plugin"` from plugin code
    diverted to stderr tagged with `[stdout-guard]` while
    shutdown reply stays on stdout.
  ...
  - **Lifecycle (1)**: double `run()` rejects with
    `PluginError`. Driven via child fixture so the dispatch
    loop does not block the test runner's stdin.
  - **Event (1)**: `Event::fromJson` validates required
    `topic` + `source`; round-trip preserves `correlation_id`
    + `metadata`; `toJson` omits absent optional fields.
  
  1 template test verifies the bash pack pipeline produces
  the canonical asset layout with synthetic SDK + vendor
  trees.
  
  Test totals: PHP SDK 14, template 1. Workspace builds
  clean. Rust ext-installer regression: 40/40. Python
  regression: 6/6 + 1/1. TS regression: 13/13 + 1/1. mdbook
  clean.
  
  New docs page `docs/src/plugins/php-sdk.md` (operator-
  author facing): architecture diagram, public API table,
  robustness defaults with focus on stdout guard rationale +
  documented `fwrite(STDOUT,...)` bypass, tarball convention,
  Composer integration walkthrough (path repository pattern,
  `symlink: false` rationale, lockfile policy, operator does
  NOT need Composer), pure-PHP constraint, CI workflow,
  install flow, smoke test, cooperative scheduling caveat
  (blocking I/O blocks dispatch — authors should
  `Fiber::suspend()` periodically). Wired under `# Plugins`
  in `docs/src/SUMMARY.md` after `typescript-sdk.md`. Cross-
  links added in Rust + Python + TS template READMEs.
  
  Two issues resolved during dev:
  
  1. **PSR-4 one-class-per-file** — initially shipped 3
     exception classes in a single `Errors.php`; PSR-4
     autoload requires filename matches class name. Split
     into `PluginError.php` + `ManifestError.php` +
     `WireError.php`.
  2. **Path repo + `minimum-stability`** — Composer
     initially refused to resolve `nexo/plugin-sdk: *`
     against an untagged path repo (treated as `dev-main`,
     blocked by default `stable` stability). Added explicit
     `version: "0.1.0"` to the SDK's `composer.json` + set
     template's `minimum-stability: dev` + `prefer-stable:
     true`. `composer validate --strict` now passes for both
     packages.
  
  Run commands:
  
  ```bash
  cd extensions/sdk-php && composer install && php tests/run-all.php
  cd extensions/template-plugin-php && composer install && php tests/test_pack_tarball.php
  ```
  
  Out of scope (tracked):
  - PHPUnit / Pest dep — stdlib `proc_open` runner.
  - ReactPHP / Amp event loops — Fibers + minimal scheduler
    sufficient.
  - Embedded PHP interpreter.
  - Packagist publish — defer until cross-language API
    stabilizes.
  - Native PHP extensions in `noarch` tarballs — Phase
    31.5.c.b follow-up.
  - PHP 7.4 / 8.0 support — Fibers require 8.1+.
  
  IRROMPIBLE refs:
  internal `crates/microapp-sdk/src/plugin.rs:514-549` (Rust
  `PluginAdapter` shape mirrored);
  internal `extensions/sdk-python/nexo_plugin_sdk/adapter.py`
  (Python `_drain_inflight` pattern → PHP `Scheduler::drain()`);
  internal `extensions/sdk-typescript/src/adapter.ts` (TS
  robustness defaults table mirrored verbatim with PHP-
  specific implementations: `ob_start` instead of
  `process.stdout.write` patch, Fiber scheduler instead of
  Promise chain, `pcntl_async_signals` instead of
  `process.on('SIGTERM')`);
  internal `extensions/sdk-typescript/src/stdout-guard.ts`
  (documented `fwrite` limitation pattern);
  internal `extensions/sdk-typescript/src/wire.ts` (PHP
  `Wire` class verbatim);
  internal `crates/core/src/agent/nexo_plugin_registry/subprocess.rs:170-219`
  (daemon spawn pipeline language-agnostic);
  internal `nexo-plugin-contract.md` (wire format implemented
  verbatim);
  internal `extensions/template-plugin-typescript/scripts/pack-tarball-typescript.sh`
  (packer pattern reused);
  internal `crates/ext-installer/src/lib.rs::resolve_release`
  (`noarch` fallback reused unchanged).
