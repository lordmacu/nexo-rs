...
- New repo `lordmacu/homebrew-nexo` with a Formula generated by
  `cargo dist`.
- Bottle (pre-built binary) for `arm64_sequoia` + `monterey` +
  `ventura` so `brew install` doesn't compile.
- Auto-PR on each release that bumps the formula version + sha.
