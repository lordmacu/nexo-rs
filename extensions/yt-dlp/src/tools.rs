...
fn info(args: &Value) -> Result<Value, ToolError> {
    ...
    let out = Command::new(bin())
        ...
        .arg(&url)
        .output().map_err(|e| ToolError{code:-32003,message:format!("spawn failed: {e}")})?;
    if !out.status.success() {
        return Err(ToolError{code:-32003,message:format!("yt-dlp failed: {}", truncate(&String::from_utf8_lossy(&out.stderr), 400))});
    }
    let meta: Value = serde_json::from_slice(&out.stdout)
        .map_err(|e| ToolError{code:-32006,message:format!("json parse: {e}")})?;
    Ok(json!({
        "ok": true,
        "id": meta.get("id"),
...
fn formats(args: &Value) -> Result<Value, ToolError> {
    ...
    let fmts: Vec<Value> = meta.get("formats").and_then(|v| v.as_array()).cloned().unwrap_or_default()
        .into_iter().map(|f| json!({
...
fn download(args: &Value) -> Result<Value, ToolError> {
    if !allow_download() {
        return Err(ToolError{code:-32041,message:"YTDLP_ALLOW_DOWNLOAD not set to `true`".into()});
    }
    let url = required_url(args, "url")?;
    let mode = args.get("mode").and_then(|v| v.as_str()).unwrap_or("video");
    let out_dir = args.get("output_dir").and_then(|v| v.as_str()).map(PathBuf::from).unwrap_or_else(default_output_dir);
    std::fs::create_dir_all(&out_dir).map_err(|e| ToolError{code:-32003,message:format!("mkdir: {e}")})?;
    let template = out_dir.join("%(title)s [%(id)s].%(ext)s");
    let template_str = template.to_string_lossy().to_string();

    let mut cmd = Command::new(bin());
    cmd.args(["--no-warnings","--no-playlist","--print","after_move:filepath","--no-progress","-o",&template_str]);

    match mode {
        "audio" => {
            let af = args.get("audio_format").and_then(|v| v.as_str()).unwrap_or("mp3");
    ...
    let output = cmd.output().map_err(|e| ToolError{code:-32003,message:format!("spawn failed: {e}")})?;
    if !output.status.success() {
        return Err(ToolError{code:-32003,message:format!("yt-dlp failed: {}", truncate(&String::from_utf8_lossy(&output.stderr), 400))});
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    let path = stdout.lines().last().map(|s| s.trim().to_string()).unwrap_or_default();
...
fn required_url(args: &Value, key: &str) -> Result<String, ToolError> {
    let s = args.get(key).and_then(|v| v.as_str()).ok_or_else(|| bad_input(format!("missing `{key}`")))?.trim().to_string();
    if !is_url(&s) { return Err(bad_input(format!("`{key}` must be http(s) URL"))); }
    Ok(s)
}
