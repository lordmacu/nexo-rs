...
fn ssh_bin() -> String { std::env::var("SSH_BIN").unwrap_or_else(|_| "ssh".into()) }
fn scp_bin() -> String { std::env::var("SCP_BIN").unwrap_or_else(|_| "scp".into()) }

fn allowlist() -> Vec<String> {
    std::env::var("SSH_EXEC_ALLOWED_HOSTS").unwrap_or_default()
        .split(',')
...
fn default_timeout() -> u64 {
    std::env::var("SSH_EXEC_TIMEOUT_SECS").ok().and_then(|s| s.parse().ok()).unwrap_or(30)
}
...
fn denied(m: impl Into<String>) -> ToolError { ToolError{code:-32041,message:m.into()} }

pub fn dispatch(name: &str, args: &Value) -> Result<Value, ToolError> {
    match name {
        "status" => Ok(status()),
        "exec" => exec(args),
        "scp_upload" => { if !allow_writes() { return Err(denied("SSH_EXEC_ALLOW_WRITES not set to `true`")); } scp_upload(args) }
...
fn check_host(host: &str) -> Result<(), ToolError> {
    if !is_host_allowed(host) {
        return Err(denied(format!("host `{host}` not in SSH_EXEC_ALLOWED_HOSTS")));
    }
    Ok(())
}
...
fn common_ssh_args(identity_file: Option<&str>) -> Vec<String> {
    ...
    if let Some(idf) = identity_file {
        v.push("-i".into()); v.push(idf.to_string());
    }
    v
}
...
fn exec(args: &Value) -> Result<Value, ToolError> {
    ...
    let timeout_secs = args.get("timeout_secs").and_then(|v| v.as_u64()).unwrap_or_else(default_timeout).min(600);
    let identity_file = args.get("identity_file").and_then(|v| v.as_str());

    let mut cmd = Command::new(ssh_bin());
    for a in common_ssh_args(identity_file) { cmd.arg(a); }
    cmd.arg(&host).arg("--").arg(&command)
        .stdout(Stdio::piped()).stderr(Stdio::piped()).stdin(Stdio::null());

    let mut child = cmd.spawn().map_err(|e| ToolError{code:-32003,message:format!("spawn failed: {e}")})?;
...
struct Waited { status: std::process::ExitStatus, stdout: Vec<u8>, stderr: Vec<u8> }

fn wait_with_timeout(child: &mut std::process::Child, timeout: Duration) -> Result<Waited, ToolError> {
    use std::io::Read;
    use std::time::Instant;
    let start = Instant::now();
    loop {
        match child.try_wait().map_err(|e| ToolError{code:-32003,message:e.to_string()})? {
            Some(status) => {
                ...
                return Ok(Waited { status, stdout, stderr });
            }
            None => {
                if start.elapsed() >= timeout {
                    let _ = child.kill();
                    let _ = child.wait();
                    return Err(ToolError{code:-32005,message:format!("timeout after {}s", timeout.as_secs())});
                }
...
fn scp_upload(args: &Value) -> Result<Value, ToolError> {
    ...
    let out = cmd.output().map_err(|e| ToolError{code:-32003,message:format!("spawn failed: {e}")})?;
    Ok(json!({
